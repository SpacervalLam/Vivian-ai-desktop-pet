...
/// 全局 AppHandle（lib.rs setup 注入，供 emit 事件的工作端点使用）
...
/// 提供桌宠图集资源文件
///
/// `path` 形如 `chibi/vivian-atlas.webp`。
...
async fn list_work_sessions(
    ...
    let arr: Vec<serde_json::Value> = sessions
        ...
        .collect();
    Ok(Json(serde_json::json!({ "sessions": arr })))
}
...
async fn get_work_session(
    ...
    match CODING_AGENT.get_session(&id) {
        ...
        None => Err((StatusCode::NOT_FOUND, format!("会话不存在: {id}"))),
    }
}

/// 向工作会话发送消息（手机端驱动工作智能体）。
#[derive(Deserialize)]
pub struct WorkMessageRequest {
    pub message: String,
}

async fn send_work_message(
    State(state): State<RemoteAppState>,
    Path(id): Path<String>,
    Json(req): Json<WorkMessageRequest>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    use crate::commands::coding_agent::CODING_AGENT;
    let Some(app) = APP_HANDLE.get().cloned() else {
        return Err((StatusCode::INTERNAL_SERVER_ERROR, "AppHandle 未注入".into()));
    };
    let router = state
        .app_state
        .model_router
        .read()
        .clone()
        .ok_or((StatusCode::SERVICE_UNAVAILABLE, "模型路由未初始化".to_string()))?;
    let tool_system = state.app_state.tool_system.clone();
    // 单轮 LLM↔工具 循环预算（与设置-工具-编程智能体最大轮次一致）
    let max_rounds = state.app_state.config.read().get_all().tools.max_coding_rounds as usize;
    match CODING_AGENT.send_message(app, id.clone(), router, tool_system, req.message, Vec::new(), Vec::new(), max_rounds, false, false) {
        Ok(()) => Ok(Json(serde_json::json!({ "session_id": id, "sent": true }))),
...
pub async fn start_server(app_state: Arc<AppState>, port: u16) {
    ...
    let app = Router::new()
        ...
        .route("/api/work/sessions", get(list_work_sessions))
        .route("/api/work/sessions/:id", get(get_work_session))
        .route("/api/work/sessions/:id/send", post(send_work_message))
...
pub fn sync_remote_server(app_state: Arc<AppState>) {
    // 初始化完成（含种子记忆注入与情绪/语义语料嵌入预加载）前不开放远程 API。
    if !app_state.is_initialized() {
        tracing::debug!("[Remote] 应用尚未完成初始化，跳过远程服务器同步");
        return;
    }
