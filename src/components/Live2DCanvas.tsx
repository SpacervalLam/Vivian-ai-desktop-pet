...
interface Live2DCanvasProps {
  ...
  /**
   * 鼠标跟随粗粒度开关：
   * - 'always'：实时看鼠标——持续注视全局光标位置（不限于窗口内）
   * - 'window'：允许跟随（实际跟随由交互事件触发，5~8s 时间窗）
   * - 'off'：禁止跟随
   */
...
export const Live2DCanvas = forwardRef<Live2DCanvasHandle, Live2DCanvasProps>(
  function Live2DCanvas(
    ...
    const mouseFollowModeRef = useRef<'always' | 'window' | 'off'>(mouseFollowMode);
    ...
    // ---- 点击压缩 / 回弹动画 ----
    // 按下时整窗（含模型）向下压缩，松开时快速弹性回弹。
    // 采用 JS 驱动的阻尼振荡：方向切换时先读取当前视觉 scaleY，
    // 从该值无缝过渡，避免快速连点导致 CSS 动画取消时的跳变。
    const PRESS_SCALE_X = 0.97;
    /** 按下时垂直压缩比例（主压缩方向为上下，向下压向窗口底端） */
    const PRESS_SCALE_Y = 0.93;
    /** 按下压缩时长（ms） */
    const PRESS_DURATION_MS = 90;
    /** 回弹振荡时长（ms）——快速、大幅缩短 */
    const REBOUND_DURATION_MS = 300;
    /** 回弹阻尼系数（越大衰减越快，趋近静止） */
    const REBOUND_DAMPING = 3.5;
    /** 回弹摆动角频率（rad/归一化时长，约 1.75 个振荡周期） */
    const REBOUND_OMEGA = 11;
    /** 当前是否处于按下压缩状态 */
    ...
    // 读取容器当前视觉 scaleY（压缩/回弹进行中的实时值）
    const readCurrentScaleY = useCallback((): number => {
    ...
    const applyPressEffect = useCallback((pressed: boolean) => {
      const el = containerRef.current;
      if (!el) return;
      if (pressActiveRef.current === pressed) return;
      pressActiveRef.current = pressed;
      // 以窗口底端为中心缩放，实现"向下压"：底端保持不变，内容向下收缩
      el.style.transformOrigin = '50% 100%';
      // 取消进行中的 spring，从当前视觉状态继续，避免跳变
      cancelSpring();
      const startScaleY = readCurrentScaleY();

      const runSpring = (targetScaleY: number, durationMs: number, overshoot: boolean) => {
        ...
        const step = (now: number) => {
          ...
          if (overshoot) {
            // 松开：阻尼振荡，超冲后衰减回到 1
            sy = 1 + (startScaleY - 1) * Math.exp(-REBOUND_DAMPING * t) * Math.cos(REBOUND_OMEGA * t);
          } else {
            // 按下：从当前值快速过渡到压缩目标（ease-out）
            const p = 1 - Math.pow(1 - t, 3);
            sy = startScaleY + (targetScaleY - startScaleY) * p;
          }
          el.style.transition = 'none';
          el.style.transform = `scaleX(${PRESS_SCALE_X}) scaleY(${sy})`;
          if (t < 1) {
            springRafRef.current = requestAnimationFrame(step);
          } else {
            // 结束：收敛到精确目标值
            springRafRef.current = null;
            el.style.transform = `scaleX(${PRESS_SCALE_X}) scaleY(${targetScaleY})`;
          }
        };
        springRafRef.current = requestAnimationFrame(step);
      };
      ...
      if (pressed) {
        runSpring(PRESS_SCALE_Y, PRESS_DURATION_MS, false);
      } else {
        runSpring(1, REBOUND_DURATION_MS, true);
      }
    }, [cancelSpring, readCurrentScaleY]);
    ...
    // ---- 音乐驱动动画 ----
    // 订阅系统音频（WASAPI 回环），把 bass/mid/beat 映射到 'music' 层
    // 随音乐摆动/节拍点头；说话/休息/无音乐时自动让位
    const musicTickRef = useMusicReactive(modelRef, {
      lipsyncRef: lipsyncRef ?? { current: null },
    });
    ...
    useEffect(() => {
      ...
      const init = async () => {
        ...
        try {
          ...
          /** 三击 → 打开心智观察器（memory 窗口）；已打开时置顶（raiseWindow） */
          const openMindInspector = () => {
            void (async () => {
              try {
                // 预加载期间禁用：角色窗口先于 Brain 初始化创建，此时心智观察器
                // 依赖的后端数据（Mind/记忆）尚未就绪，打开只会得到空壳/报错页
                const ready = await invoke<boolean>('is_initialized');
                if (!ready) return;
                const existing = await WebviewWindow.getByLabel('memory');
                if (existing) {
                  // 已打开：提升到屏幕顶端（临时 topmost 突破桌宠遮挡，失焦自动降回）
          ...
          const tickerCallback = () => {
            ...
            musicTickRef.current?.();
            // 鼠标跟随窗口过期（或跟随被关闭）后，将注视目标回中。
            // focusController 会残留鼠标最后位置的偏移，SDK updateFocus 每帧将其
            // 累加到头部/眼球参数上，导致模型一直看向固定方向；回中后由
            // useMicroPresence 的视线游移接管，呈现随机张望。
            // 'always' 模式（实时看鼠标）不回中，保持注视光标。
            if (
              mouseFollowModeRef.current === 'off' ||
              (mouseFollowModeRef.current === 'window' && !isInteractionFollowActive())
            ) {
              const m = modelRef.current;
              if (m) {
          ...
          void (async () => {
            try {
              const unlisten = await listen<{
                ...
                let fy: number;
                if (mode === 'always') {
                  // 实时看鼠标：全局光标位置换算为注视方向（不限于窗口内）。
                  // 以窗口中心为原点、半窗宽/高为尺度，窗口边缘恰好满偏转，
                  // 窗外光标继续延伸后钳制到 ±1（看向光标所在方位）。
                  const cxn = (cursor_x - (window_x + window_w / 2)) / (window_w / 2);
                  const cyn = (cursor_y - (window_y + window_h / 2)) / (window_h / 2);
                  fx = Math.max(-1, Math.min(1, cxn));
                  fy = Math.max(-1, Math.min(1, -cyn));
                } else {
                  ...
                  fx = Math.max(-1, Math.min(1, cx * 2 - 1));
                  fy = Math.max(-1, Math.min(1, -(cy * 2 - 1)));
                }
                const fc = (m as unknown as {
                  internalModel?: { focusController?: { focus?: (x: number, y: number) => void } };
                }).internalModel?.focusController;
                if (fc && typeof fc.focus === 'function') {
                  ...
                  const px = ((fx + 1) / 2) * (m.width || 1);
                  const py = ((1 - fy) / 2) * (m.height || 1);
                  m.focus(px, py);
                }
              });
              if (destroyed) {
