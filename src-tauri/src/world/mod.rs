...
/// 构建 SunriseSunset：优先使用天气 API 返回的日出日落小时，
/// 未配置经纬度或 API 未返回时回退到本地算法。
///
/// 昼夜判断按当前系统本地小时与日出日落小时实时比较：
/// API 的 `is_day` 是 fetch 时刻的快照，且随天气缓存（TTL 默认 1 小时）延迟更新，
/// 直接采用会导致昼夜转换发生在"缓存刷新时刻"而非真实日出/日落时刻，
/// 表现为日出日落提醒在不该出现的时间点触发。实时比较保证转换精确发生在
/// 日出/日落小时（tick 粒度内）。
fn build_sunrise_sunset(
    config: &WorldConfig,
    weather: Option<&WeatherSnapshot>,
    now: &chrono::DateTime<chrono::Local>,
) -> Option<SunriseSunset> {
    if let Some(w) = weather {
        if let (Some(sunrise_hour), Some(sunset_hour)) = (w.sunrise_hour, w.sunset_hour) {
            use chrono::Timelike;
            let hour_f = now.hour() as f64 + now.minute() as f64 / 60.0;
            let is_daytime = hour_f >= sunrise_hour && hour_f < sunset_hour;
            return Some(SunriseSunset {
                sunrise_hour,
                sunset_hour,
                is_daytime,
            });
    ...
    if config.latitude.is_some() && config.longitude.is_some() {
        ...
        return time_perception::compute_sunrise_sunset(lat, lon, now);
    }
    None
}
...
pub struct WorldStateProvider {
    ...
    /// 前台窗口切换监听器（按角色 ID 唯一，由各角色 ActivityJournal 注入）
    ///
    /// 设置保存会重建 Brain。若这里只追加，旧 listener 的闭包会继续持有旧
    /// ActivityJournal / WorldState，导致每次 reinitialize 都永久保留一套角色状态。
    foreground_listeners: RwLock<HashMap<String, ForegroundListener>>,
    /// 缓存的用户在场状态快照
...
impl WorldStateProvider {
    pub fn new(config: WorldConfig) -> Self {
        Self {
            ...
            foreground_listeners: RwLock::new(HashMap::new()),
    ...
    /// 设置前台窗口切换监听器（由各角色 Brain 初始化时调用）
    ///
    /// 全局共享的 WorldStateProvider 支持多个监听器，每个角色的 ActivityJournal
    /// 都会收到 title 变化事件，独立记录到各自的 user_behaviors.json。
    /// 回调应保持轻量（避免阻塞事件循环）。
    pub fn set_foreground_listener(
        &self,
        char_id: impl Into<String>,
        listener: ForegroundListener,
    ) {
        self.foreground_listeners
            .write()
            .insert(char_id.into(), listener);
    }

    /// 移除配置中已不存在角色的监听器，释放其闭包捕获的角色状态。
    pub fn retain_foreground_listeners(
        &self,
        active_character_ids: &std::collections::HashSet<String>,
    ) {
        self.foreground_listeners
            .write()
            .retain(|char_id, _| active_character_ids.contains(char_id));
    }
    ...
    pub fn snapshot(&self, seconds_since_last_interaction: Option<f64>) -> WorldSnapshot {
        ...
        // 日出日落：优先使用天气 API 返回的时间（更准确、随地理/季节自动更新），
        // 天气不可用或未返回时回退到本地算法计算。
        let sunrise_sunset = build_sunrise_sunset(&config, weather.as_ref(), &now);
    ...
    /// 读取最近一次轮询的系统指标缓存（10s 轮询；缓存为空时同步采集一次）
    ///
    /// 供主动交互（内存压力提醒等）在 tick 中直接查询，无需构造完整 WorldSnapshot。
    pub fn system_metrics(&self) -> Option<SystemMetrics> {
        if let Some(m) = self.cached_system.read().clone() {
            return Some(m);
        }
        self.refresh_system();
    ...
    /// 低频、确需进程明细的时刻调用；常规轮询/会话 prompt 一律不走这里。
    pub fn top_memory_processes(&self, n: usize) -> Vec<ProcessMemoryAgg> {
        self.system_collector
            .lock()
            .top_memory_processes(n, std::process::id())
    }
...
mod tests {
    ...
    fn foreground_listener_is_replaced_per_character_and_removed_when_inactive() {
        ...
        let calls = Arc::clone(&stale_calls);
        provider.set_foreground_listener(
            "vivian",
            Arc::new(move |_| {
                calls.fetch_add(1, Ordering::SeqCst);
            }),
        );
        let calls = Arc::clone(&current_calls);
        provider.set_foreground_listener(
            "vivian",
            Arc::new(move |_| {
                calls.fetch_add(1, Ordering::SeqCst);
        ...
        let calls = Arc::clone(&removed_calls);
        provider.set_foreground_listener(
            "nana",
            Arc::new(move |_| {
                calls.fetch_add(1, Ordering::SeqCst);
        ...
        let active = ["vivian".to_string()].into_iter().collect();
        provider.retain_foreground_listeners(&active);
        provider.update_foreground(ForegroundWindowSnapshot {
            title: "memory regression test".to_string(),
