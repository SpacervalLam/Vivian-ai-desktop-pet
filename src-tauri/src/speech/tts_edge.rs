...
///
/// 与官方 voices/list 接口实测一致（2026-08 校验）：
/// zh-CN 仅 6 个标准音色（晓涵/晓墨/晓轩等为 Azure 专属，Edge 免费服务不支持，
/// 使用会导致服务静默关闭连接无音频返回）；en-US 共 17 个。
/// 此列表同时作为 resolve_voice 的有效性校验依据，必须与 API 实际返回保持同步。
const EDGE_VOICES: &[(&str, &str, &str)] = &[
    ...
    // 英文（美国）
    ("en-US-AvaNeural", "Ava (女)", "en-US"),
    ("en-US-AvaMultilingualNeural", "Ava Multilingual (女)", "en-US"),
    ("en-US-AndrewNeural", "Andrew (男)", "en-US"),
    ("en-US-AndrewMultilingualNeural", "Andrew Multilingual (男)", "en-US"),
    ("en-US-EmmaNeural", "Emma (女)", "en-US"),
    ("en-US-EmmaMultilingualNeural", "Emma Multilingual (女)", "en-US"),
    ("en-US-BrianNeural", "Brian (男)", "en-US"),
    ("en-US-BrianMultilingualNeural", "Brian Multilingual (男)", "en-US"),
    ("en-US-AnaNeural", "Ana (女)", "en-US"),
    ("en-US-AriaNeural", "Aria (女)", "en-US"),
    ("en-US-ChristopherNeural", "Christopher (男)", "en-US"),
    ...
    ("en-US-RogerNeural", "Roger (男)", "en-US"),
    ("en-US-SteffanNeural", "Steffan (男)", "en-US"),
    // 日语
...
impl EdgeTtsBackend {
    ...
    async fn fetch_voices_from_api(client: &reqwest::Client) -> VivianResult<Vec<VoiceInfo>> {
        ...
        for voice in voices {
            ...
            // 接口无 LocalName/DisplayName 字段，从 FriendlyName 提取人名
            // （"Microsoft Xiaoxiao Online (Natural) - Chinese (Mainland)" → "Xiaoxiao"）
            let name = voice
                .get("FriendlyName")
                .and_then(|v| v.as_str())
                .map(|f| {
                    let core = f.split(" - ").next().unwrap_or(f);
                    let core = core.strip_prefix("Microsoft ").unwrap_or(core);
                    core.strip_suffix(" Online (Natural)").unwrap_or(core).trim().to_string()
                })
                .filter(|s| !s.is_empty())
            ...
            // 仅保留标准 zh-CN（排除 liaoning/shaanxi 方言）、en-US、ja-JP
            if language == "zh-CN" || language == "en-US" || language == "ja-JP" {
                result.push(VoiceInfo { id, name, language });
            }
        }

        // zh-CN 优先（与内置 EDGE_VOICES 顺序一致），其次 en-US、ja-JP，同语言内按名称排序
        let locale_order = |lang: &str| match lang {
            "zh-CN" => 0,
            "en-US" => 1,
            "ja-JP" => 2,
            _ => 3,
        ...
        result.sort_by(|a, b| {
            locale_order(&a.language)
                .cmp(&locale_order(&b.language))
