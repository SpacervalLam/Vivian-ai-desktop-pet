...
/** 将角色 ID 首字母大写以匹配 public 目录名（vivian → Vivian） */
const capitalizeCharId = (id: string): string =>
  id.charAt(0).toUpperCase() + id.slice(1);

/** AI 头像（按角色加载对应 icon.png，缺省回退 favicon） */
const AiAvatar: React.FC<{ size: number; characterId?: string }> = ({ size, characterId }) => {
  const src = characterId ? `/${capitalizeCharId(characterId)}/icon.png` : '/favicon.ico';
  return (
    <img
      src={src}
...
const ChatWindow: React.FC = () => {
  ...
  const handleSendImage = useCallback(async () => {
    try {
      ...
      if (!selected || Array.isArray(selected)) return;
      setBottomPanel('none');
      await invoke('send_image_message', { sourcePath: selected, characterId: privateCharId ?? undefined });
    } catch (e) {
      console.warn('[ChatWindow] 发送图片失败:', e);
                  ...
                  return (
                    ...
                    >
                      <div style={{ position: 'relative', flexShrink: 0 }}>
                        <img
                          src={`/${capitalizeCharId(c.id)}/icon.png`}
                          alt={c.name}
                          draggable={false}
                          style={{
