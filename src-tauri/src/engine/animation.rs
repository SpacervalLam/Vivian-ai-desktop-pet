...
fn on_motion_timer_end(
    ...
    if current_motion.is_looping {
        // 循环动作重新启动
        restart_motion(inner, current_motion);
    } else {
        // 处理队列：启动下一个排队动作
        process_queue_static(inner);
    }
}
...
fn restart_motion(
    ...
    if !motion.is_looping && motion.duration > 0.0 {
        ...
        let handle = spawn_timer(duration_ms, move || {
            on_motion_timer_end(inner_clone);
        });
        if let Some(h) = handle {
...
fn process_queue_static(
    ...
    // 启动动作（对应 start_motion 的完整逻辑：设置当前动作、触发开始回调、启动自动结束定时器）
    start_motion_static(inner, next_motion);
}
