/**
 * 音频分析器 — 移植自 Petra 的 AudioAnalyzer.ts（MIT）
 *
 * 订阅后端 WASAPI 回环捕获推来的 `audio:pcm`（Vec<f32> 单声道波形），
...
export class AudioAnalyzer {
  ...
  /**
   * 停止并释放：关闭 AudioContext 释放音频线程与 AnalyserNode 资源。
   * 实例丢弃前（禁用 / 卸载）必须调用，否则每个 new AudioContext() 都
   * 留下一个永不关闭的音频上下文（浏览器对同窗口 context 数量有限制）。
   */
  close() {
    this.stop();
