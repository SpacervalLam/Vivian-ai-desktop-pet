...
pub struct Re2Advisor {
    name: String,
    /// 启用 Re2 的任务类型列表（默认 ["reasoning", TASK_WORK_AGENT]）。
...
impl Re2Advisor {
    pub fn new() -> Self {
        Self {
            name: "re2".to_string(),
            // 工作智能体此前复用 reasoning，天然被覆盖；它改用专属任务类型后
            // 必须显式列进来，否则 Re2 会静默失效。
            enabled_tasks: RwLock::new(vec![
                "reasoning".to_string(),
                crate::providers::base::TASK_WORK_AGENT.to_string(),
            ]),
        }
    }
