...
fn spawn_pty(
    ...
    unsafe {
        ...
        // 4.5 绑定到全局 Job Object：主程序 panic / 强杀 / 系统崩溃时，OS 关闭 Job 句柄
        //     会自动连带终止 powershell.exe 及其所有子孙进程，避免孤儿终端
        let _ = crate::utils::job_object::assign_process(pi.hProcess.0 as isize);
...
/// 应用退出时一次性终止所有 PTY 会话
///
/// 由 `lib.rs` 的 `RunEvent::ExitRequested` 处理器调用。即使遗漏调用，
/// `spawn_pty` 已把子进程绑定到全局 Job Object，OS 关闭 Job 句柄时
/// 会自动连带终止 powershell.exe，本函数仅是确保 `terminal:exit` 事件
/// 能广播给前端、句柄能确定性释放的"快速路径"。
pub fn kill_all_sessions(app: &AppHandle) {
    let ids: Vec<String> = SESSIONS.lock().keys().cloned().collect();
    if ids.is_empty() {
        return;
    }
    tracing::info!("[terminal] 退出清理：终止 {} 个 PTY 会话", ids.len());
    for id in ids {
        kill_session(app, &id);
    }
}
