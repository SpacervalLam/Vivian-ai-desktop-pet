 ...
 * 跨窗口 toast 内容去重。
 *
 * 背景：toast 窗口是**按角色私有**的（label = `${charId}_toast`），而 `app.emit` 是
 * 全局广播。于是任何「被广播、又没带角色归属」的事件，都会让每只桌宠的 toast 窗口
 * 各渲染一条同样的文案——视觉上就是同一句话上下叠两条。
 *
 * 归属路由（payload 带 character_id + 接收侧按归属过滤）能消灭已知来源，但它依赖
 * 「每个发送侧都记得标注归属」这一约定：新增一个广播监听、或漏写一次归属，
 * 重复就会重新出现。本模块提供与来源无关的兜底，让「同一内容在屏幕上只存在一条」
 * 成为硬约束。
 *
 * 判定规则（两个窗口都按同一规则计算，故无需协商即可收敛到同一条）：
 * 1. 本窗口在时间窗内已呈现过同一指纹 → 放弃。
 * 2. 另一窗口已宣告呈现同一指纹，且其优先级**更高** → 让位（撤下自己那条）。
 *
 * 优先级用 `winsOver` 定义，必须是全序——否则两个窗口会互相认为对方更低而各留一条。
 */
