...
// 仅引入类型，echarts/mermaid 运行时按需动态 import，避免增大主包
...
const NotebookPage: React.FC = () => {
  ...
  // 预览宿主：用 Shadow DOM 渲染笔记 HTML，隔离样式并规避 iframe 继承宿主 CSP 导致内联样式被拦截的问题
  const previewRef = useRef<HTMLDivElement>(null);
  // 记录当前 Shadow DOM 内的 ECharts 实例，用于切换笔记时释放
  const chartInstancesRef = useRef<echarts.ECharts[]>([]);

  // 在 Shadow DOM 内初始化图表(ECharts)与流程图(Mermaid)
  const initDiagramsInShadow = useCallback(async (root: ShadowRoot) => {
    const chartEls = Array.from(root.querySelectorAll<HTMLElement>('.nb-chart[data-option]'));
    const mermaidEls = Array.from(root.querySelectorAll<HTMLElement>('.nb-mermaid'));
    if (!chartEls.length && !mermaidEls.length) return;

    // ECharts：解析 data-option 后按需初始化（懒加载，仅含图表时引入）
    if (chartEls.length) {
      try {
        const echartsMod = await import('echarts');
        chartEls.forEach((el) => {
          let opt: unknown;
          try { opt = JSON.parse(el.getAttribute('data-option') || 'null'); } catch { return; }
          if (!opt) return;
          const fallback = el.querySelector('.nb-chart-fallback');
          if (fallback) fallback.remove();
          try {
            const chart = echartsMod.init(el);
            chart.setOption(opt as echarts.EChartsOption);
    ...
    // Mermaid：将 .nb-mermaid 的文本内容渲染为 SVG（懒加载）
    if (mermaidEls.length) {
      try {
        ...
        mermaidEls.forEach((el) => {
          ...
          mermaid
            .render('mmd-' + Math.random().toString(36).slice(2, 8), code)
            .then((res) => { el.innerHTML = res.svg; })
            .catch(() => { el.innerHTML = '<div class="nb-mermaid-error">⚠️ 流程图解析失败</div>'; });
        });
      } catch { /* ignore */ }
    }
  }, []);
  ...
  const loadNoteHtml = useCallback(async (charId: string, noteId: string) => {
    setLoadingHtml(true);
    try {
      const result = await invoke<{ html: string; note_id: string; font_path?: string | null }>(
        'get_notebook_html',
        { charId, noteId },
      );
      let htmlContent = result.html;
      // 本地手写字体：srcDoc 的 base 是 about:srcdoc，相对路径无法解析，
      // 故将相对 `fonts/...` 改写为 Tauri asset URL（convertFileSrc）。
      if (result.font_path) {
        const fontUrl = convertFileSrc(result.font_path);
        htmlContent = htmlContent.replace(/url\('fonts\/ma-shan-zheng\.woff2'\)/g, `url('${fontUrl}')`);
        htmlContent = htmlContent.replace(/url\("fonts\/ma-shan-zheng\.woff2"\)/g, `url("${fontUrl}")`);
      }
      setHtml(htmlContent);
    } catch (e) {
      console.error('加载笔记 HTML 失败:', e);
      setHtml('<div style="padding:40px;text-align:center;color:#999;">加载失败</div>');
    } finally {
  ...
  // 将笔记 HTML 渲染进 Shadow DOM：样式隔离 + 不依赖 iframe 对 CSP 的继承
  useEffect(() => {
    // 释放上一份笔记的图表实例，避免内存/监听泄漏
    chartInstancesRef.current.forEach((c) => { try { c.dispose(); } catch { /* ignore */ } });
    ...
    // renderer 输出的是完整 HTML 文档（含 <head>/<style>/<body>）。直接 innerHTML 塞进
    // Shadow DOM 时，`:root` 与 `body`/`html` 选择器不会匹配 shadow root，导致所有 CSS
    // 变量失效、笔记失去样式。这里先解析出 CSS 与 body 内容，把选择器改写为 `:host`
    // 后再注入，使样式在 Shadow DOM 内正确生效。
    const parsed = new DOMParser().parseFromString(html, 'text/html');
    const css = Array.from(parsed.head.querySelectorAll('style'))
      .map((s) => s.textContent || '')
      .join('\n')
      .replace(/:root\b/g, ':host')
      // 仅改写独立的选择器（排除 .card-body 这类带前缀的类名）
      .replace(/(?<![\w.-])body(?=\s*\{)/g, ':host')
      .replace(/(?<![\w.-])html(?=\s*\{)/g, ':host');
    const bodyHtml = parsed.body ? parsed.body.innerHTML : html;
    // ShadowRoot 无 insertAdjacentHTML，先写入 body 内容再追加 style 元素
    ...
    void initDiagramsInShadow(root);
  }, [html, initDiagramsInShadow]);
