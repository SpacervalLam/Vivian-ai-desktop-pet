...
export default function MessageBannerWindow() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      unlisten = await listen<BannerPayload>('wechat:message_banner', (e) => {
        ...
        // 头像 URL 异步解析（dev=Vite 路径，release=model 协议加密 URL）
        void resolveAvatarUrl(p.character_id).then((avatarSrc) => {
  ...
  useEffect(() => {
    ...
    // 隐藏后主动冻结渲染进程：横幅是低频窗口，空闲时它只是一个常驻的
    // WebView2 渲染进程（约 50-120MB）。冻结后 JS 挂起、进程工作集可被回收。
    // 后端在 emit 前会先解冻（commands::window::emit_message_banner），
    // 因此冻结不会丢事件。
    void (async () => {
      try {
        await win.hide();
        await invoke('freeze_window_webview', { label: 'message_banner' });
      } catch {
        /* 冻结失败仅意味着内存照旧，不影响功能 */
      }
    })();
  ...
  const handleBannerClick = useCallback(async (item: BannerItem) => {
    ...
    // 1. 如果已有该 chat 窗口：展开微信抽屉 + 发 navigate 事件切换到私聊
    ...
    // 2. 不存在则创建为右缘抽屉，URL 携带 private_char_id 自动跳转
    try {
