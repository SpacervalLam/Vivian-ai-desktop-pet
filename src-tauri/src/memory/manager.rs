...
/// 常驻内存的条目硬上限。
///
/// 桌宠是长期不关机的进程，`data.entries` 只在容量满时被"标记"为已归档，
/// 从不真正移出内存，几个月下来会累积成千上万条并全部在启动时反序列化。
const MAX_RESIDENT_ENTRIES: usize = 20_000;

/// 触发卸载后回落到的水位（留一段缓冲，避免每次 add 都触发卸载）
const RESIDENT_EVICT_TARGET: usize = 18_000;
...
impl MemoryManager {
    pub fn new(config: &AppConfig, char_id: &str) -> VivianResult<Self> {
        ...
        let pending_conflicts = crate::utils::fs::load_json_or_backup::<
            Vec<super::conflict::PendingConflict>,
    ...
    fn save_pending_conflicts(&self) -> VivianResult<()> {
        ...
        drop(queue);
        crate::utils::fs::write_atomic(&path, &s)?;
        Ok(())
    }
    ...
    pub async fn process_pending_conflicts(
        ...
        for mut pc in batch {
            ...
            let (new_context, old_context) = {
                let inner = self.inner.read();
                let describe = |id: &str, fallback: &str| {
                    inner
                        .id_index
                        .get(id)
                        .and_then(|&i| inner.data.entries.get(i))
                        .map(|m| format!(
                            ...
                            m.timestamp,
                            m.importance,
                            m.metadata.get("source").and_then(|v| v.as_str()).unwrap_or("unknown"),
                            m.reinforcement,
                            m.disputation,
                        ))
                ...
                (
                    describe(&pc.new_memory_id, &pc.new_content),
                    describe(&pc.old_memory_id, &pc.old_content),
                )
            };
            match llm.arbitrate(&new_context, &old_context, pc.similarity).await {
                ...
                Ok(super::conflict::ArbitrationOutcome::ReplaceOld) => {
                    ...
                    if let Some(&idx) = inner.id_index.get(&pc.old_memory_id) {
                ...
                Ok(super::conflict::ArbitrationOutcome::MergeSupersede(merged)) => {
                    ...
                    // The merged text changed, so replace the new item's stale vector.
                    if let Ok(embedding) = inner.embedding.embed(&merged) {
                        let new_meta = inner
                            .id_index
                            .get(&pc.new_memory_id)
                            .and_then(|&idx| inner.data.entries.get(idx))
                            .map(|item| {
                                (
                                    item.importance,
                        ...
                        if let Some((importance, memory_type, timestamp)) = new_meta {
                            inner.vector_store.delete_by_memory_id(&pc.new_memory_id);
                            let _ = inner.vector_store.add(MemoryVector {
                                doc_id: pc.new_memory_id.clone(),
                                memory_id: pc.new_memory_id.clone(),
    ...
    /// 批量更新召回遥测：增加 visit_count、刷新 last_visit_at、重算 heat_score。
    ///
    ...
    ///
    /// 反馈闭环：检索命中（`bump_visits`）仅更新访问次数与热度，会反映到后续健康度；
    ...
    async fn add_memory_inner(
        ...
        match &conflict_outcome {
            ...
            ConflictOutcome::ReplaceOld { old_id } => {
                if let Some(&idx) = inner.id_index.get(old_id) {
                    if let Some(old) = inner.data.entries.get_mut(idx) {
    ...
    pub async fn save_context_with_metadata(
        ...
        long_term_metadata: Option<serde_json::Value>,
    ) -> VivianResult<()> {
        let char_id = self.char_id.clone();
        ...
        if let Some(ltm) = long_term_memory {
            ...
            if !trimmed.is_empty() {
                ...
                let default_ltm_meta = serde_json::json!({
                    "channel": "direct",
                    "speaker": "user",
                    "listener": char_id,
                    "knowledge_source": "llm_extracted",
                });
                let meta = long_term_metadata.unwrap_or(default_ltm_meta);
                if let Err(e) = self
                    .add_memory_with_metadata(
                        trimmed,
                        MemoryType::LongTerm,
                        importance_user.clamp(0.4, 0.8),
                        tags,
                        meta,
                    )
                    .await
    ...
    pub async fn soft_delete_memory(&self, id: &str, reason: &str) -> VivianResult<()> {
        // 先把可恢复副本原子落盘，再移除规范记录；任一步失败都至多留下
        // 可去重的双份数据，不会出现主库和回收站同时没有该记忆的窗口。
        let backup_item = {
            let inner = self.inner.read();
            inner
                .id_index
        ...
        let Some(backup_item) = backup_item else {
            return Ok(());
        };
        self.recycle_bin.push(backup_item, reason);
        self.recycle_bin.save()?;
    ...
    /// Advance the evidence-based archival clock once per natural day.
    ///
    /// Returns ids whose sustained negative evidence reached the archive threshold.
    /// The caller performs soft archival after this lock is released.
    pub fn advance_evidence_archival(&self) -> VivianResult<Vec<String>> {
        let now = current_timestamp();
        let mut inner = self.inner.write();
        let mut changed = false;
        let mut archive_ids = Vec::new();
        for entry in inner.data.entries.iter_mut().filter(|e| !e.consolidated) {
            let before_days = entry.sub_zero_days;
            let before_date = entry.sub_zero_last_increment_date.clone();
            let (_, should_archive) = super::evidence::maybe_mark_sub_zero(entry, now);
            changed |= before_days != entry.sub_zero_days
                || before_date != entry.sub_zero_last_increment_date;
            if should_archive {
                archive_ids.push(entry.id.clone());
            }
        }
        if changed {
            inner.save_to_disk()?;
    ...
    pub async fn search_memories(
        ...
        //
        // 多数情况下没有任何条目被过滤掉，此时直接借用原切片，避免把整张表
        // 深拷贝一份（条目数上万时单次检索就会复制几十 MB）。
        let needs_filter = inner
            .data
            .entries
            .iter()
        ...
        let all_entries: Cow<[MemoryItem]> = if needs_filter {
            ...
            Cow::Borrowed(&inner.data.entries)
        };
    ...
    pub async fn search_memories_filtered(
        ...
        // 与 search_memories 同理：无需过滤时直接借用，省掉整表深拷贝
        let needs_filter = inner
            .data
            .entries
            .iter()
    ...
    pub async fn search_memories_with_options(
        ...
        // Step 1~4 全是同步操作，放在同一作用域内即可直接借用条目切片，
        // 省掉原先的整表深拷贝（条目上万时单次检索要复制几十 MB）。
        // 这个块必须在 Step 6 之前结束：后验证会 .await，
        // 而 RwLockReadGuard 不是 Send，不能跨 await 持有。
        let expanded = {
            let inner = self.inner.read();
            if inner.data.entries.is_empty() {
                return Ok(Vec::new());
...
impl MemoryManagerInner {
    ...
    fn add_entry(&mut self, item: MemoryItem) -> VivianResult<()> {
        ...
        // Preserve caller supplied/event timestamps. `MemoryItem::new` already assigns
        // the current time for ordinary writes, so resetting it here would destroy
        // legitimate timestamp overrides.
        self.data.entries.push(item);
        // 超限后卸载完全归档的条目（只卸内存，磁盘行保留）
        self.evict_archived_from_memory();
        // 条目库是规范存储：返回成功前必须提交。向量/图谱等派生索引即使稍后
        // 失败也可从这里重建，避免进程在节流窗口内退出时丢掉新记忆。
        self.save_to_disk()?;
        Ok(())
    }

    /// 常驻条目超过 [`MAX_RESIDENT_ENTRIES`] 时，把"完全归档"的条目从内存卸载。
    ///
    /// 完全归档 = `consolidated` 且未标记 `summarized`：这类条目连检索候选都进不去
    /// （见 `search_memories` 的 `!e.consolidated || is_summarized(e)` 过滤），
    /// 留在内存里是纯浪费。
    ///
    /// 只卸载内存副本，**磁盘行保留**——因此必须同步把这些 id 从 `persisted` 摘除，
    /// 否则下一次 `save_to_disk` 会把"内存里没有而磁盘上有"判定为删除，造成数据丢失。
    /// 卸载后如再被访问，重启时 `load_all` 会重新载入。
    fn evict_archived_from_memory(&mut self) {
        let total = self.data.entries.len();
        if total <= MAX_RESIDENT_ENTRIES {
            return;
        }

        let mut evicted: Vec<String> = Vec::new();
        let mut remaining: Vec<MemoryItem> = Vec::with_capacity(total);
        for entry in self.data.entries.drain(..) {
            let fully_archived = entry.consolidated && !is_summarized(&entry);
            let still_over = total.saturating_sub(evicted.len()) > RESIDENT_EVICT_TARGET;
            if fully_archived && still_over {
        ...
        if evicted.is_empty() {
            // 没有任何可安全卸载的条目（都还没归档），宁可暂时超限也不丢数据
        ...
        self.rebuild_index();
        tracing::info!(
            "[MemoryManager] 常驻条目 {} 超过上限 {}，已从内存卸载 {} 条完全归档记忆（磁盘保留，重启后可重新载入）",
            total,
