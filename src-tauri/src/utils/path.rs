...
pub fn get_resource_dir() -> PathBuf {
    ...
    if let Some(exe) = std::env::current_exe().ok().and_then(|p| p.parent().map(|d| d.to_path_buf())) {
        ...
        if has_model_dir(&exe) {
            return exe;
        }
        // 新配置：resources/<模型名>/
        let resources = exe.join("resources");
        if resources.exists() && has_model_dir(&resources) {
            return resources;
        }
        // 旧配置兼容：_up_/public/<模型名>/
        let old = exe.join("_up_").join("public");
        if old.exists() && has_model_dir(&old) {
            return old;
        }
    }
    ...
    for _ in 0..5 {
        ...
        if public.exists() && has_model_dir(&public) {
            return public;
        }
        if !dir.pop() {
            break;
...
/// 检查目录下是否有任意 桌宠 模型子目录（含 model_manifest.json）
fn has_model_dir(dir: &std::path::Path) -> bool {
