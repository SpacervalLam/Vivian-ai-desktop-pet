...
pub async fn proactive_tick(
    ...
    // ── World Entity State 桥接：把 idle_seconds 翻译为用户在场/离开信号 ──
    {
        ...
        if idle < 60.0 {
            // Away → Present 转换：生成「你不在时发生了什么」回归摘要
            if let Some(ret) = brain.world_state.mark_user_present() {
                let router = brain.router.clone();
                let memory = brain.memory.clone();
                let cid = char_id.clone();
                let app_handle = app.clone();
                tauri::async_runtime::spawn(async move {
                    let _ = crate::proactive::recap::generate_return_recap(
                        &cid, &router, &memory, ret.away_secs, &app_handle,
                    )
                    .await;
                });
            }
