...
use crate::providers::base::{
    parse_stream_usage, BaseProvider, ChatResponse, LLMRequest, StreamEvent, ToolDefinition,
};
...
pub struct ModelRouter {
    ...
    /// 工作智能体模型热切换覆盖（reasoning 任务）
    ///
    /// 用户为工作智能体选择的 provider 实例；请求 reasoning 时优先使用，
    /// 替换 routing_matrix.reasoning 的默认配置。`None` 表示未覆盖。
...
impl ModelRouter {
    ...
    fn build_reasoning_override(
        ...
        if cfg.model.trim().is_empty() || cfg.endpoint.trim().is_empty() || cfg.api_key.trim().is_empty()
        {
            tracing::warn!(
    ...
    fn effective_request_reasoning(&self, requested: ReasoningPreference) -> ReasoningPreference {
    ...
    /// 按请求级推理偏好设置所有 provider 的覆盖（请求前调用）。
    ///
    /// 传播到 main_provider / task_providers / 工作智能体覆盖模型；
    /// 请求结束后用 None 恢复（交由服务端默认）。
    /// provider 构造请求体时按模型能力映射为各家 wire 字段。
    fn set_reasoning_all(&self, pref: Option<ReasoningPreference>) {
        if let Some(p) = self.main_provider.as_ref() {
            p.set_reasoning_pref(pref);
        }
        for provider in self.task_providers.values() {
            provider.set_reasoning_pref(pref);
        }
        if let Some(p) = self.reasoning_override.read().as_ref() {
            p.set_reasoning_pref(pref);
        }
    }
    ...
    pub async fn generate(&self, mut request: LLMRequest) -> VivianResult<String> {
        loop {
            ...
            // 应用请求级推理偏好（调用后恢复）
            self.set_reasoning_all(Some(self.effective_request_reasoning(request.reasoning)));
    ...
    pub async fn generate_stream(
        ...
        loop {
            ...
            // 应用请求级推理偏好（provider 在构造请求体时读取，流建立后立即恢复）
            self.set_reasoning_all(Some(self.effective_request_reasoning(request.reasoning)));
            let effective_schema = if self.strict_broken.load(Ordering::Relaxed) {
                None
            } else {
