...
import EvolutionSection from './graph/EvolutionSection';
import GraphOverviewBar from './graph/GraphOverviewBar';
import type { TypeChipDef } from './graph/GraphOverviewBar';
...
const formatTimeLabel = (ts: number, now: number, t: TFunction): string => {
  const diff = now - ts;
  const minute = 60 * 1000;
  const hour = 60 * minute;
  const day = 24 * hour;
  if (diff < minute) return t('mind_inspector.common.just_now');
  if (diff < hour) return t('mind_inspector.common.minutes_ago', { n: Math.floor(diff / minute) });
...
const NodeShape: React.FC<NodeShapeProps> = ({ type, color, size, x, y, isCore, expanded }) => {
  ...
  switch (type) {
    ...
    case 'belief': {
      // 信念节点图标：当前为原始 1/3，再扩大 1.5 倍 → 原始 1/2
      const beliefSize = size * 0.5;
      const br = beliefSize / 2;
      return (
        <g {...shapeProps}>
          <rect x={x - br} y={y - br} width={beliefSize} height={beliefSize} fill={pastel(color)} rx={3} stroke={GPAPER.card} strokeWidth={2.5} />
          <rect x={x - br + 3} y={y - br + 3} width={beliefSize - 6} height={(beliefSize - 6) * 0.34} fill={hexToRgba('#ffffff', 0.4)} rx={2} />
        </g>
      );
    }
    case 'diary': {
...
const GraphPage: React.FC = () => {
  ...
  const handleDateSearch = useCallback(
    (dateStr: string) => {
      // 支持 2024-01-15 / 2024/1/15 / 1月15日 / 20240115
      const now = new Date();
      let ts: number | null = null;
      const norm = dateStr.replace(/[年月/.]/g, '-').replace(/日/g, '');
      const parts = norm.split('-').filter(Boolean).map((p) => parseInt(p, 10));
      if (parts.length >= 2 && parts.every((p) => Number.isFinite(p))) {
        const [y, m, d, h] = parts;
        const year = y > 1000 ? y : now.getFullYear();
        ts = new Date(year, (m ?? 1) - 1, d ?? 1, h ?? 12).getTime();
      ...
      if (ts == null || !Number.isFinite(ts) || skeleton.length === 0) return;
      let bestIdx = -1;
      let bestDt = Infinity;
      skeleton.forEach((p, i) => {
  ...
  // 便签定位：直接使用鼠标在屏幕/视口中的实时坐标，换算到图谱容器内容坐标
  const tooltipAnchor = (() => {
    if (!hoveredPos) return null;
    const container = containerRef.current;
    if (!hoverClient || !container) return null;
    const rect = container.getBoundingClientRect();
    const rawLeft = hoverClient.x - rect.left;
    const rawTop = hoverClient.y - rect.top + container.scrollTop;
    const offset = 16;
    const placeRight = rawLeft < rect.width / 2;
    const left = placeRight ? rawLeft + offset : rawLeft - offset;
    const transform = placeRight
      ? 'translate(0, -50%)'
      : 'translate(-100%, -50%)';
    return { left: `${left}px`, top: `${rawTop}px`, transform };
  })();
  ...
  return (
    ...
    >
      ...
      <div
          ...
          {filteredNodes.map((node) => {
            ...
            return (
              <g
                ...
                onMouseMove={(e) => setHoverClient({ x: e.clientX, y: e.clientY })}
                onMouseLeave={() => {
                  setHoveredNode(null);
                  setHoverClient(null);
                }}
        ...
        {/* Tooltip - 按节点类型使用不同形状的便签（角色节点 user/agent 与长内容类型 reading 不显示，改用点击弹窗；其余类型均呼出详情便签） */}
        {tooltipAnchor && hoveredGraphNode
          ...
          const timeLabel = formatTimeLabel(hoveredGraphNode.timestamp, Date.now(), t);
                ...
                <div style={{
                  ...
                  {childCount > 0 && (
                    <div style={{ marginTop: 6, fontSize: 12, color: '#7C3AED', fontFamily: 'system-ui, sans-serif' }}>
                      {expandedSummaryIds.has(hoveredGraphNode.id) ? t('mind_inspector.graph.toggle_collapse') : t('mind_inspector.graph.toggle_expand')} {t('mind_inspector.graph.child_chat_count', { count: childCount })}
                    </div>
                  )}
                </div>
              </div>
      ...
      {/* 角色成长记录：人格自进化覆盖层（已生效调整 + 酝酿中候选） */}
      <EvolutionSection character={character} />
      ...
      {modalNode && (() => {
        ...
        const typeLabel = t(`mind_inspector.graph.${NODE_TYPE_KEYS[ntype]}`);
        const timeLabel = formatTimeLabel(modalNode.timestamp, Date.now(), t);
        return (
          <div
            onClick={(e) => {
              e.stopPropagation();
