                ...
                Some(offset) => {
                    ...
                    if let Some(tag_close) = Self::find_tag_close(&text, tag_start) {
                        ...
                        if after_lt < bytes.len()
                            && matches!(bytes[after_lt], b'e' | b'm')
                        {
...
mod tests {
    ...
    /// 只认 `<e>` / `<m>` 两种标签字母；其它字母（含已下线的 `<s>` 贴纸标签）
    /// 一律按普通文本原样保留。这是扫描器的既有约定，不是遗漏。
    #[test]
    fn test_unknown_tag_letter_kept_as_text() {
        let (tags, cb) = collect_tags();
        let mut scanner = InlineTagScanner::new(cb);
        let clean = scanner.feed(r#"送你 <s name="flowers"/>"#);
        assert_eq!(clean, r#"送你 <s name="flowers"/>"#);
