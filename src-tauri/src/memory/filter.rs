...
fn tag_tokens(text: &str) -> Vec<Tag<'_>> {
    super::tokenize::jieba().tag(text, true)
}
...
impl MemoryFilter {
    ...
    pub fn calculate_memory_weight(memory: &MemoryItem) -> f64 {
        ...
        let (base_weight, half_life_hours) = if Self::is_tagged_long_term(memory)
        ...
        // Exponential decay preserves a weak signal instead of collapsing old but
        // valid memories to an exact zero after a fixed cutoff.
        let time_decay = 0.5_f64.powf(hours / half_life_hours);
