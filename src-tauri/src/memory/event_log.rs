...
impl EventLog {
    ...
    pub async fn advance_sentinel(&self, event_id: &str) -> VivianResult<()> {
        ...
        // 原子落盘（自带 fsync + UUID 临时名）；放入 spawn_blocking 避免阻塞 tokio 工作线程
        let sentinel_path = self.sentinel_path.clone();
        tokio::task::spawn_blocking(move || crate::utils::fs::write_atomic(&sentinel_path, &json))
            .await
            .map_err(|e| VivianError::Serialization(format!("sentinel 落盘任务失败: {e}")))??
            ;
    ...
    pub async fn compact_if_needed(&self) -> VivianResult<usize> {
        ...
        // 原子替换（sync_all + UUID 临时名 + 父目录创建 + 操作系统写穿）
        let new_content = kept.join("\n") + "\n";
        let log_path = self.log_path.clone();
        tokio::task::spawn_blocking(move || crate::utils::fs::write_atomic(&log_path, &new_content))
            .await
            .map_err(|e| VivianError::Serialization(format!("事件日志落盘任务失败: {e}")))??;
        ...
        tokio::task::spawn_blocking(move || {
            crate::utils::fs::write_atomic(&sentinel_path, &sentinel_json)
        })
        .await
        .map_err(|e| VivianError::Serialization(format!("sentinel 落盘任务失败: {e}")))??;
...
impl Reconciler {
    ...
    pub async fn reconcile(&self) -> VivianResult<usize> {
        ...
        for event in events {
            ...
            match handler.handle(&event.payload).await {
                Ok(changed) => {
                    ...
                    // 仅记录最后一个已应用事件，落盘推迟到循环结束后做一次
                    last_applied_id = Some(event.event_id.clone());
                }
                Err(e) => {
        ...
        // 一次落盘：原实现为每条事件各写一次 sentinel。这里推进到本次重放最后一个已应用
        // 事件（语义等价：中途暂停时仍保留此前已应用事件的进度）。
        if let Some(id) = last_applied_id {
            self.log.advance_sentinel(&id).await?;
        }
