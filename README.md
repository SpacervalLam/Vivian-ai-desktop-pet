...
内置的认知调试器前端工具（Memory 窗口，默认全屏大小），可从系统托盘菜单打开，或**长按任意一个桌宠 1 秒**（按住不拖动）打开——按住满 0.1 秒就开始预加载窗口，长按成立时松手即见，提供 3 个顶级导航页面可视化智能体内部状态与工作台：
...
- **用户直接交互触发**（纯前端规则：`ChibiPetCanvas` 渲染层上报单击/双击/戳毛了 + `App.tsx` 手势判定，零 LLM 开销）：
  ...
  | **戳毛了** | 5 秒窗口内累计 7 点（每戳一下 1 点，与上一戳间隔 <350ms 的猛戳额外 1 点） | **当场**切 `angry` 表情 + **窜开**（只挪窗口、不播走路动画，滑到远处一个随机落点）+ 对应的 AI 台词；并进入 2.5 秒「气头上」，期间每戳一次续期，停手满 2.5 秒才消气、从零重新攒 |
  ...
  **长按 0.1 秒就开始加载窗口，但要等长按成立才让它上屏**：窗口加载（WebView 冷启 + React 挂载 + 首屏数据）是这条链上最慢的一环，而长按判定要整整 1 秒——等到成立才去建窗口，用户松手后还得再干等一截（模型实测：页面挂载 120ms 时要等 355ms，冷启慢到 1.5s 时要等 1735ms）。所以按住满 0.1 秒就去把窗口建出来加载，绝大多数情况下长按成立时它已经就绪，**松手即见**。三条约束缺一都会让「预热」变成「误开」：① 预热期间窗口一律不上屏——预热 URL 带 `hidden=1` 关掉子窗口「渲染完两帧就 show」的兜底，子窗口认出预热参数后也只藏不播，连 1.6 秒的兜底显形都不挂（兜底的前提是「这窗口本来就该出现」，而预热窗口该不该出现还没答案）；② 长按成立才放行，且要等子窗口回执「监听已挂好」再发显形事件——「窗口建好」不等于「子窗口接得住事件」，两者之间隔着一整个页面冷启，回执丢了还有 1.2 秒上限兜底，宁可晚一点也不让长按没反应；③ 中途松手就销毁，加载没跑完的也一并打断（正在创建的由预热链自己收尾），不留一份白占内存的后台 webview；但已被最小化 / hide 的窗口只「认领」不销毁——那种窗口里有用户可能没保存的输入。长按成立时如果窗口本来就开在屏上，长按仍是「收起」，不预热。
...
- **MoodCue 规则映射层**（`mood_cue.rs`）：在完整心理管道之外维护一套纯规则驱动的 Mood→表情映射，从 MoodSnapshot 直接映射到一个**限时**表情（规则表给出的名字经 [`src/chibi/animations.json`](file:///g:/vivian-rs/src/chibi/animations.json) 的 `expression_aliases` 归一为 Q 版动作）。由前端每 ~4s 一次的 `auto_expression_tick` 驱动（`update_mood_state`），不是预留能力。
  ...
  - **第三层 中度疲劳**（无强情绪时倦意浮上表面）：昏昏欲睡(fatigue>72+arousal<0.4)→`dizzy`
  - **第四层 效价-唤醒空间**：兴奋(高唤醒+正效价)→`smug`、满怀期待→`happy`、安心惬意→`blink`、焦虑→`think`、不高兴→`angry`、情绪低落(valence<-0.3)→`dizzy`、委靡无力(valence<-0.45+arousal<0.25)→`dizzy`、好奇观察→`think`
  - **第五层 关系背景调制**：老朋友默契(关系分>75)→`smug`、疏离旁观(关系分<15)→不出表情
  - **兜底**：平静待机→不出表情（`idle`）
...
- **微存在感**：CSS keyframes 驱动待机呼吸（`chibi-idle-breathe`，4.1s 循环）与影子同步缩放；眨眼为独立序列帧（6 帧，随机 3.2-7.5s 间隔，仅 idle 态播放）；`prefers-reduced-motion` 时动画时长压至 1ms；`presenceState` store（online/busy/rest/offline）驱动表现切换——`busy` 播「掏出手机 → 看手机」帧序列并**留在原位**（不主动打扰由姿态本身表达，退到角落只剩 48px 就什么都看不到了），只有 `rest` 退到屏幕角落
...
- **位移前后的转身过渡**：有水平分量的避让位移是「转身 → 走动/滑动 → 回身」三段——`playTurn` 转完**停在侧身末帧**（不回落基准姿态，避免「转身完成」与「起步走动」之间闪一帧正面待机），滑动与走动共用同一段时间轴（走动收尾即窗口到位），收尾由 `playTurnBack` 从末帧倒放回正面、再回落 `idle`。打断路径同样收尾（`abortToRest`）：用户按下时交给精灵自身的 mousedown 处理，其余打断（获得焦点、卸载）把角色送回基准姿态，不留侧身或抬腿的定格
...
- **边界**：朝向由**实际**位移定（贴边截断后抽签方向可能与实际反号）；离屏幕边不足一个完整步长时朝里走；两边都放不下 `WALK_DISTANCE_MIN_PX`(140px) 就放弃这趟，不硬塞一段比转身动画还短的位移。走完由 `playTurnBack` 回身并回落 `idle`
...
| 桌宠不再自己走动（自主漫步） | 静息期本就长（48–120 秒，均值约 84 秒），且**姿态不是 `idle` 时永不漫步** | 节奏/距离常数集中在 `ChibiPetCanvas.tsx` 顶部（`WALK_REST_*` / `WALK_DISTANCE_*` / `WALK_BLOCKED_RETRY_*` / `WALK_STARTUP_*`）；被占用（说话 / 全屏 / 避让 / 被按住）时**不消耗静息期**，只按 8–14 秒重试，腾出手就接着走。这条门槛曾经还会被心情基调误伤（后端下发非 idle 基调后漫步彻底停摆），持续基调撤掉后那条路径已不存在——现在只有「真的在播某个动作」才会挡住漫步 |
| 连点桌宠之后它自己跑开了 | **预期行为**：戳毛了（5 秒窗口内攒够 7 点）之后角色会窜到远处一个随机落点 | 只想让它别跑就正常节奏点（两次双击、每 4 秒一下永远不会攒满）；它窜的过程中**按住**（够 250ms）可以随时抢回来——点一下是逗它、不算抓住。距离下限/上限是 `ChibiPetCanvas.tsx` 顶部的 `FLEE_MIN_DISTANCE_PX` / `FLEE_MAX_DISTANCE_PX`，生气脸亮多久是 `FLEE_ANGER_BEAT_MS`。它只挪窗口、不播走路动画，所以生气脸不会在位移途中被换掉 |
| 连点之后它生气了但窗口没动 | 拖动会话把位移拽回去了：每次按下都开一次窗口拖动，而后端记的光标偏移是按下那一刻采的，追踪线程每 60ms 就用它把窗口钉回按下时的位置 | 已修（逃离期间延后开启窗口拖动）。若再次出现，先确认真机上按的是不是真的「连点」——合成点击（按下即抬起）不会暴露这个问题，按一下要按住 60~120ms 才复现 |
