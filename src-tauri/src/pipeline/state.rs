...
pub struct PipelineState {
    ...
    /// 桌宠自控动作指令（chat 任务产出 桌宠 模型控制，由 ControlActionExecutor 执行）
    #[serde(default)]
    ...
    /// 日程通知信号评估（在 FastSemantic 阶段同步计算，纯规则）
    ///
    /// 判定用户输入是否像一份转来的、含时间安排的通知，命中时注入助理引导段。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub schedule_assessment: Option<crate::emotion::ScheduleAssessment>,
