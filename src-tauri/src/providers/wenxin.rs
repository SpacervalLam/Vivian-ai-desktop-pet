...
use crate::providers::base::{parse_stream_usage, BaseProvider, ProviderBase, StreamEvent};
...
impl BaseProvider for WenxinProvider {
                ...
                while let Some(pos) = buffer.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        ...
                        if let Ok(json) = serde_json::from_str::<Value>(data) {
                            if let Some(usage) = parse_stream_usage(&json["usage"]) {
    ...
    fn provider_identity(&self) -> String {
        format!("wenxin:{}@{}", self.base.model, self.base.base_url)
    }
