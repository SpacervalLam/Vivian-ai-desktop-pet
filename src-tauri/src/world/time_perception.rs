...
pub fn compute_sunrise_sunset(
    ...
    let year = now.year();
    // 闰年修正
    let n = day_of_year + if is_leap(year) { 0.0 } else { 0.0 };
...
fn sunrise_time(
    ...
    compute_event(lat, lon, n, -0.833, now)
}
...
fn sunset_time(
    ...
    compute_event(lat, lon, n, -0.833, now).map(|sr| {
        // 日落与日出对称于正午，简化用正午推算
...
fn compute_event(
    ...
    // 太阳平近点角近似
    let day_angle = 2.0 * std::f64::consts::PI * (n - 1.0) / 365.0;
    // 时差（equation of time，分钟）
    let eot = 9.87 * (2.0 * day_angle).sin() - 7.53 * day_angle.cos() - 1.5 * day_angle.sin();
    // 太阳赤纬（度）
    let decl = 23.45 * day_angle.sin();
    let lat_rad = lat.to_radians();
    let decl_rad = decl.to_radians();
    let cos_h = (zenith.to_radians().cos()
    ...
    let tz = timezone_offset(now);
    let noon = 12.0 - lon / 15.0 + tz;
    let event = noon - h - eot / 60.0;
    Some(event.clamp(0.0, 24.0))
}
