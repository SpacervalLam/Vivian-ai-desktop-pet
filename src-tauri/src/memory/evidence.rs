...
//! - `sub_zero_days` 独立归档倒计时：score<=归档阈值累计达 14 天才真正归档
...
///
/// 当 `score <= ARCHIVE_THRESHOLD` 且当天未计数过 → `sub_zero_days += 1`。
...
pub fn maybe_mark_sub_zero(memory: &mut MemoryItem, now: f64) -> (u32, bool) {
    ...
    if score > ARCHIVE_THRESHOLD && memory.rebuttal_grace_remaining == 0 {
        // 已离开归档区间且无宽限期 → 重置计数器
