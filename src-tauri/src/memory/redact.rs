...
/// PII 原文驻留容量上限（FIFO 驱逐）：该表存的是脱敏命中的**原文**（银行卡号、
/// 密码等），既无上限会随处理文本量无界累积，也让敏感原文长期驻留内存。
/// 超限后淘汰最早的条目——占位符本身仍可读，仅丢失还原能力，隐私优先可接受。
const TRACKER_STORE_CAP: usize = 4096;
...
static TRACKER_STORE: Lazy<RwLock<TrackerStore>> = Lazy::new(|| RwLock::new(TrackerStore::default()));
    ...
    for pattern in PATTERNS.iter() {
        for mat in pattern.regex.find_iter(text) {
            ...
            let tracker_id = compute_tracker_id(pattern.pii_type, matched);
            {
                ...
                if let std::collections::hash_map::Entry::Vacant(e) = store.map.entry(tracker_id.clone()) {
                    e.insert(matched.to_string());
                    store.order.push_back(tracker_id.clone());
                    // FIFO 驱逐最旧条目，保持驻留容量有界
                    while store.order.len() > TRACKER_STORE_CAP {
                        let evicted = store.order.pop_front();
...
pub fn tracker_lookup(tracker_id: &str) -> Option<String> {
    TRACKER_STORE.read().map.get(tracker_id).cloned()
}
