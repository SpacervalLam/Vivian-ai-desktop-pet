...
//! `set_emotion_expression` 通过 emit 事件通知前端 桌宠 应用表情。
...
///
/// 只写后端心理状态，不产生事件推送：前端调用后用
/// [`get_current_mood`] 拉取快照刷新 store。
#[tauri::command]
pub fn psychology_micro_tick(
    ...
    if let Ok(instance) = state.get_character(character_id.as_deref()) {
        instance.brain.psychology.micro_tick();
    }
    Ok(())
}
...
///
/// 将单一情绪标签 + 强度转换为 FACS 通道值，供前端直接写入 桌宠 模型。
