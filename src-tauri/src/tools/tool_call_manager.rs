...
impl ToolCallManager {
    ...
    /// 工具系统引用（供外部构造 MultiStepExecutor 等使用）
    pub fn tool_system(&self) -> Arc<ToolSystem> {
        Arc::clone(&self.tool_system)
    }

    /// 当前上下文快照（供外部构造 MultiStepExecutor 等使用）
        ...
        Fut: std::future::Future<Output = Option<String>>,
    {
        ...
        let max_feedback_rounds = self.max_iterations.saturating_sub(1).max(1).min(4);
    ...
    fn build_feedback_prompt(&self, results: &[ToolCallResult], last_response: &str) -> String {
        ...
        for r in results {
            ...
            if r.success {
                if let Some(data) = &r.result {
                    ...
                    let truncated: String = data_str.chars().take(self.feedback_history_chars).collect();
        ...
        lines.push(format!("上一轮 AI 响应：{}", last_response));
        lines.push(String::new());
        if any_goal_completed {
            lines.push("**用户目标已通过工具调用完成。** 请直接以角色人设口吻简短告知用户结果，".to_string());
            lines.push("**禁止再调用任何工具**（尤其是搜索/查询类工具），不要尝试\"进一步帮助用户\"。".to_string());
            lines.push("- 禁止使用客服/助手语气（如\"好的\"\"已为您完成\"\"这就帮您\"），必须保持傲娇、嘴硬心软的说话方式。".to_string());
        } else {
            lines.push("请根据以上工具执行结果，以角色人设口吻回复用户：".to_string());
            lines.push("- 若任务已完成，用符合人设的语气简短告知结果（不要再调用工具）；".to_string());
            lines.push("- 若需要继续调用工具，请输出工具调用 JSON。".to_string());
            lines.push("- 禁止使用客服/助手语气（如\"好的\"\"已为您完成\"\"这就帮您\"），必须保持傲娇、嘴硬心软的说话方式。".to_string());
        }
...
///
/// 与 ClaudeCode 的 ToolSearchTool 等价：
...
///
/// 实现说明：持有工具快照 `Vec<Arc<dyn Tool>>` 而非 `Arc<ToolSystem>`，
/// 因为工具注册只在启动时做一次，之后不变，快照足够。这样避免 ToolSystem 自引用 Arc 的复杂性。
pub struct ToolSearchTool {
    tools_snapshot: Arc<Vec<Arc<dyn Tool>>>,
}

impl ToolSearchTool {
    pub fn new(tools_snapshot: Arc<Vec<Arc<dyn Tool>>>) -> Self {
        Self { tools_snapshot }
    }
    ...
    pub fn search(&self, query: &str, max_results: usize) -> (String, Vec<String>) {
        let all_tools = &self.tools_snapshot;
