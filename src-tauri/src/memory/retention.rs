...
//! - 默认规则提供按类型的年龄/数量候选阈值，最终由统一健康度决定是否软归档
...
impl MemoryRetentionGuard {
    ...
    pub fn is_expired(memory: &MemoryItem, rules: &[MemoryExpirationRule]) -> bool {
        ...
        let health = super::lifecycle::health_score(memory, now);

        // 年龄/类型规则只产生候选，统一健康度做最终判定，避免独立机制
        // 在仍有强证据、近期使用或高有效重要度时机械归档。
        ...
        // QuadraticDecay is a retrieval/health signal, not sufficient evidence for
        // irreversible expiry. Type TTL/count rules above own archival decisions.
    ...
    pub async fn cleanup_expired(
        ...
        let evidence_archive_ids = manager.advance_evidence_archival()?;
        let mut deleted = 0usize;
        let mut archived_ids: HashSet<String> = HashSet::new();
        for id in evidence_archive_ids {
            manager.archive_memory(&id)?;
            archived_ids.insert(id);
            deleted += 1;
        }

        let memories: Vec<MemoryItem> = manager
            .get_all_memories()
            .await?
            .into_iter()
        ...
        for memory in &memories {
            if Self::is_expired(memory, &self.rules) {
                manager.archive_memory(&memory.id)?;
                archived_ids.insert(memory.id.clone());
                deleted += 1;
            }
        }
        ...
        for memory in memories
            .iter()
            .filter(|m| !m.protected && !archived_ids.contains(&m.id))
        {
            let mem_type = extract_memory_type(memory);
            ...
            if let Some(max_count) = rule.max_count {
                if let Some(items) = by_type.get(&rule.memory_type) {
                    if items.len() > max_count {
                        ...
                        // 所有数量淘汰统一以健康度为主；配置为按价值淘汰时用
                        // evidence+importance 作次级键，否则用创建时间作次级键。
                        sorted.sort_by(|a, b| {
                            let health_order = super::lifecycle::health_score(a, now)
                                .partial_cmp(&super::lifecycle::health_score(b, now))
                                .unwrap_or(std::cmp::Ordering::Equal);
                            if health_order != std::cmp::Ordering::Equal {
                                return health_order;
                            }
