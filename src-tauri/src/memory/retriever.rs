...
///
/// recency + relevance + importance 三因子加权，各分量先 min-max 归一化到 [0,1]
/// 再加权求和，本实现扩展为五因子。
...
pub fn hybrid_search(
    ...
    // Bind ids to real items before reranking/post-graph compensation. The old
    // implementation reranked placeholders with empty content and timestamps.
    let mut reranked = rerank(fused, query, entries);
...
fn apply_post_graph_compensation(
    ...
    for (item, score) in compensated {
        reranked.push(RetrievalHit {
            ...
            // 补位候选只能补足覆盖面，不能凭一套 0..1 的后图谱分数
            // 压过 RRF（通常约 0.01..0.06）的真实命中。锚定尾部命中并
            // 保证严格不高于它，避免量纲不一致导致无关候选霸榜。
            fused_score: tail_score * (0.60 + 0.39 * score.total.clamp(0.0, 1.0)),
...
/// 实体/专名补充召回路（查询增强）
///
/// 从 query 中提取显著词：两字以上纯 CJK 词，或三字以上非纯 CJK 专名。
...
fn entity_arm_search(entries: &[MemoryItem], query: &str, k: usize) -> Vec<(String, f64)> {
    ...
    let significant: Vec<&String> = query_tokens
        ...
        .filter(|t| {
            ...
            let all_cjk = t.chars().all(is_cjk);
            (all_cjk && len >= 2) || (!all_cjk && len >= 3)
        })
        .collect();
...
fn rerank(
    ...
    let mut hits: Vec<RetrievalHit> = fused
        .into_iter()
        .filter_map(|(id, bm25, vec, graph, fused_score)| {
            let item = (*index.get(id.as_str())?).clone();
            Some(RetrievalHit {
                result_type: RetrievalResultType::from_memory_item(&item.memory_type, &item.episode_id),
                item,
                bm25_score: bm25,
                vector_score: vec,
                graph_score: graph,
    ...
    // 对 query 完全包含的命中加权：RRF 分数量级约 0.016-0.066，0.05 的奖励足以
    // 让"内容完整包含查询"的强信号命中升至顶部，而不再是无意义的 +0.001。
...
///
/// 显著词 = 两字以上纯 CJK 词，或三字以上非纯 CJK 专名（如 "AlenTinn"）。
...
fn entity_token_hit(query_tokens: &std::collections::HashSet<String>, item: &MemoryItem) -> bool {
    ...
    query_tokens.iter().any(|t| {
        let len = t.chars().count();
        let all_cjk = t.chars().all(is_cjk);
        let significant = (all_cjk && len >= 2) || (!all_cjk && len >= 3);
...
pub fn attach_items_with_weights(
    ...
    let raw_rec_range = rec_max - rec_min;
    let rec_range = raw_rec_range.max(1e-9);
    ...
    for (idx, raw_rel, raw_rec, raw_imp, raw_hb, raw_ns) in &raw_components {
        ...
        // 创建时间仅差几十秒时不应被 min-max 拉伸成 0 与 1；那会让测试
        // 构造顺序或同一轮的写入先后压过真正的相关性。recency 差异不足
        // 约千分之一时视为同档。
        let norm_rec = if raw_rec_range < 1e-3 {
            0.0
        } else {
            (raw_rec - rec_min) / rec_range
        };
        ...
        if let Some(obj) = meta.as_object_mut() {
            ...
            // Compatibility alias used by temporal decay and prompt confidence.
            obj.insert("combined_score".to_string(), serde_json::json!(final_score));
            obj.insert(
    ...
    // 小样本豁免：min-max 归一化在候选 ≤ 5 条时分数失真（极值被拉平），
    // 且少量候选本就稀缺，全部保留。
    if weights.min_score > 0.0 && hits.len() > 5 {
        let before = hits.len();
...
/// MMR 多样化的相关性/多样性权衡系数。
///
/// λ=1 纯相关度排序，λ 越小越强调多样性。0.7 表示相关度权重约为多样性的 2.3 倍：
/// 保证最相关的记忆仍然优先，同时显著抑制近重复条目挤占注入名额。
pub const MMR_LAMBDA: f64 = 0.7;

/// MMR（最大边际相关性）多样化重排。
///
/// 对按相关度降序的候选列表做贪心重排：每轮选择
/// `λ × relevance − (1−λ) × max_sim(候选, 已选集合)` 最大的一条，
/// 惩罚与已选内容高度相似的条目，让 Top-K 覆盖更多不同侧面而非近重复堆叠。
///
/// - 相关度项用位置归一化（首条 1.0，线性递减到接近 0），不依赖内部分数字段
/// - 相似度项用 Jaccard token 重叠（jieba 分词），零嵌入成本
/// - 只重排前 `keep` 条的选择顺序，其余按原序拼接，不丢条目
/// - `lambda >= 1.0` 时退化为纯相关度排序（短路）
pub fn mmr_diversify(mut items: Vec<MemoryItem>, lambda: f64, keep: usize) -> Vec<MemoryItem> {
    let n = items.len();
    if n <= 1 || keep >= n || lambda >= 1.0 {
        return items;
    }

    // 预分词：content + description + tags 合并口径（与 entity_token_hit 一致）
    ...
    // 相关度按位置归一化：i=0 → 1.0，i=n-1 → 1/n（保底 > 0）
    let relevance = |i: usize| 1.0 - (i as f64) / (n as f64);

    while selected.len() < keep && !remaining.is_empty() {
        // 首条直接取相关度最高（位置 0 必然在 remaining 首位）
    ...
    let mut diversified: Vec<MemoryItem> = Vec::with_capacity(n);
    let selected_set: std::collections::HashSet<usize> = selected.iter().copied().collect();
    for idx in selected {
        diversified.push(items[idx].clone());
    }
    // 未选中的按原序拼接（后续截断或扩窗使用，不丢弃）
...
mod tests {
    ...
    #[test]
    fn entity_arm_recalls_two_char_cjk_entity() {
        let entries = vec![
            make_item("m1", "用户计划下个月去上海工作", 0.5),
            make_item("m2", "用户最近在研究编译器", 0.5),
        ];
        let hits = entity_arm_search(&entries, "上海", 5);
        assert_eq!(hits.first().map(|hit| hit.0.as_str()), Some("m1"));
    }
    ...
    fn semantic_boost_written_to_metadata() {
        ...
        assert_eq!(
            attached[0].item.metadata.get("fused_score").and_then(|v| v.as_f64()),
            attached[0].item.metadata.get("combined_score").and_then(|v| v.as_f64()),
            "新旧评分键必须保持一致，避免下游读取不同分数"
        );
    }

    #[test]
    fn deterministic_chinese_retrieval_eval_meets_quality_gate() {
        let entries = vec![
            make_item("location", "用户现在住在上海浦东", 0.7),
            make_item("food", "用户明确说自己不吃香菜", 0.7),
            make_item("exam", "用户下个月准备报名考研", 0.7),
            make_item("pet", "用户的小猫名字叫团子", 0.7),
            make_item("noise1", "今天讨论了数据库索引", 0.5),
            make_item("noise2", "最近天气变化很快", 0.5),
            make_item("noise3", "角色喜欢动画和游戏", 0.5),
        ];
        let cases = [
            ("上海浦东住址", "location"),
            ("香菜饮食忌口", "food"),
        ...
        for (query, expected) in cases {
            let hits = hybrid_search(&entries, query, None, None, None, 20, 7);
            let ranked = attach_items(hits, &entries, query);
            if let Some(rank) = ranked.iter().position(|h| h.item.id == expected) {
                if rank < 3 {
                    hit_at_3 += 1;
                }
        ...
        assert!(hit_at_3 as f64 / n >= 0.75, "deterministic hit@3 回归");
        let mrr = reciprocal_rank / n;
        assert!(mrr >= 0.50, "deterministic MRR 回归: {mrr:.3}");
    }
