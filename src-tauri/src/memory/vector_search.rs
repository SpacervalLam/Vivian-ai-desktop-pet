...
//!
//! ## 索引策略
//! 全量索引：除 TemporaryContext 与低重要性 InnerMonologue 外，所有持久记忆
//! 均建立向量索引（召回优先；sqlite-vec 暴力 KNN 在万级向量内为毫秒级）。
...
impl MemoryVectorStore {
    ...
    /// 批量添加向量：单事务写入全部行，供播种/补建/全量重建使用，
    /// 避免逐条提交事务的开销（96 条种子从 96 次事务降为 1 次）。
    /// 返回成功写入的条数。
    pub fn add_batch(&self, vecs: &[MemoryVector]) -> VivianResult<usize> {
        if vecs.is_empty() {
            return Ok(0);
        }
        if let Some(q) = &self.qdrant {
            q.upsert_points(&self.collection, vecs, &self.model_name)?;
            return Ok(vecs.len());
        }
        let conn = self.conn.lock();
        ...
        let tx = conn
            ...
            .map_err(|e| VivianError::Tool(format!("开启批量事务失败: {}", e)))?;
        let mut count = 0usize;
        for vec in vecs {
            let emb_bytes: Vec<u8> = vec.embedding.iter().flat_map(|f| f.to_le_bytes()).collect();
            conn.execute(
                "INSERT OR REPLACE INTO memory_vectors(doc_id, memory_id, content, importance, memory_type, timestamp, model)
                 VALUES(?1, ?2, ?3, ?4, ?5, ?6, ?7)",
                rusqlite::params![
            ...
            .map_err(|e| VivianError::Tool(format!("批量向量插入失败: {}", e)))?;
            conn.execute("DELETE FROM vec_memory WHERE doc_id=?1", [&vec.doc_id])
                .map_err(|e| VivianError::Tool(format!("向量清理失败: {}", e)))?;
            conn.execute(
                "INSERT OR REPLACE INTO vec_memory(rowid, embedding, doc_id)
        ...
        tx.commit()
            .map_err(|e| VivianError::Tool(format!("批量事务提交失败: {}", e)))?;
        Ok(count)
    }
...
/// 判断记忆是否应建立向量索引
///
/// 全量索引策略：除 TemporaryContext（纯临时上下文）与 InnerMonologue（高频
/// 主观内心流，低重要性时噪声多）外，所有持久记忆一律入向量库。窄准入
/// （imp≥0.7 白名单）曾导致两类召回损失：日常对话记忆（imp 0.25-0.6）从不
/// 进入向量检索；播种全量写入的种子向量被 IndexDrift 按"孤儿"误删。
pub fn should_index(importance: f64, memory_type: &MemoryType) -> bool {
    match memory_type {
        MemoryType::TemporaryContext => false,
        MemoryType::InnerMonologue => importance >= INDEX_IMPORTANCE_THRESHOLD,
        _ => true,
    }
}
...
mod tests {
    ...
    fn test_should_index_by_importance() {
        ...
        assert!(should_index(0.3, &MemoryType::ShortTerm));
    ...
    fn test_should_index_by_type() {
        ...
        assert!(should_index(0.1, &MemoryType::General));
        assert!(should_index(0.25, &MemoryType::ShortTerm));
        // TemporaryContext 永不索引；InnerMonologue 仅高重要性时索引
        assert!(!should_index(0.9, &MemoryType::TemporaryContext));
        assert!(!should_index(0.3, &MemoryType::InnerMonologue));
        assert!(should_index(0.8, &MemoryType::InnerMonologue));
    }

    #[test]
    fn test_add_batch_atomic_write() {
        let path = std::env::temp_dir().join(format!("vec_batch_{}.db", uuid::Uuid::new_v4()));
        let dim = 4usize;
        let store = MemoryVectorStore::new(path.clone(), dim, "hashing-test").unwrap();
        let vecs: Vec<MemoryVector> = (0..5)
            .map(|i| make_vec(&format!("doc{i}"), &format!("mem{i}"), vec![i as f32, 1.0, 0.0, 0.0]))
            .collect();
        let n = store.add_batch(&vecs).unwrap();
        assert_eq!(n, 5);
        assert_eq!(store.len(), 5);
        // 重复写入同 doc_id 不产生重复行（INSERT OR REPLACE + doc_id 去重）
        let n2 = store.add_batch(&vecs).unwrap();
        assert_eq!(n2, 5);
        assert_eq!(store.len(), 5);
        // KNN 可命中
        let hits = store.search(&[4.0, 1.0, 0.0, 0.0], 3);
        assert!(!hits.is_empty());
        assert_eq!(hits[0].0, "doc4");
        drop(store);
