...
///
/// 这些工具涉及隐私敏感操作，默认必须经前端用户确认后才能执行：
/// - 文件读写 6 个：read_file / write_file / edit_file / list_directory /
///   search_files / grep
/// - 屏幕感知 2 个：take_screenshot / screenshot_analyze
/// - 任务/待办删除 2 个：cancel_scheduled / delete_todo
///
...
pub async fn check_tool_permission(
    ...
    // 5.5 浏览器可信来源：导航到高信任白名单站点免确认，收敛信任边界。
    ...
    // 7. 委托给工具自身的权限检查
    //    隐私敏感工具（文件 6 + 屏幕 2 + 任务/待办 2）的 check_permissions 会返回 ask
...
pub fn requires_permission(
    ...
    // 隐私敏感工具（文件 6 + 屏幕 2 + 任务/待办 2）始终需要进入权限检查流程
