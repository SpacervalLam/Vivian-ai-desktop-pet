...
//!   时效信息 → 调 `network::web` 搜索缝隙 → LLM 汇总成结构化知识 → 写入
...
async fn run_knowledge_acquisition(
    ...
    // 内容发现维护：探针周期（promote 升级兴趣）+ 低库存补货
    // 探针生成自带 6 小时节流，补货受库存门槛控制，常态为轻量维护
    crate::discovery::maintenance_pass(char_id).await;
    ...
    for tw in topics.iter().take(MAX_TOPICS_PER_ACQUISITION) {
        // Step 2: 搜索（委托 network::web 服务缝隙，按配置选择 provider 链）
        let request =
            WebSearchRequest::new(&tw.topic).with_max_results(SEARCH_RESULTS_PER_TOPIC);
        ...
        match tw.intent {
            ...
            TopicIntent::Share => {
                ...
                // 多平台候选（发现引擎按主题定向搜寻 + LLM 评估）优先竞争本次分享；
                // 惊喜级（≥0.75）直接胜出，无惊喜级时回退网页搜索 → LLM 选链接
                let bili_delight =
                    crate::discovery::acquire_delight_candidates(char_id, &tw.topic).await;
                let best_bili = bili_delight
                    .iter()
                    .max_by(|a, b| {
                        b.score
                            .partial_cmp(&a.score)
                    ...
                    .cloned();
                if let Some(item) = best_bili {
                    let follow_up = crate::discovery::expression_for(char_id, &item).await;
                    share_link_to_wechat(
                        ...
                        &item.title,
                        &item.reason,
                        crate::discovery::platform_display_name(&item.platform),
                        &follow_up,
                    )
                    .await;
                    // 记入推荐账本（平台:ID 键），避免后续被动推荐重复推同一条
                    let mut store = crate::discovery::store::ContentStore::load(char_id);
                    store.mark_recommended(&[format!("{}:{}", item.platform, item.bvid)]);
                    store.save(char_id);
                    proactive.mark_knowledge_share_expressed();
                    shared += 1;
                    share_summaries.push(format!("「{}」", item.title));
                    tracing::info!(
                        "[Presence:{}] {} 高分候选胜出，链接已通过微信面板分享：{}（score={:.2}）",
                        char_id,
                        item.platform,
                        item.title,
                        item.score
...
async fn decide_topics_with_intent(
    ...
    // 数据源 3：兴趣探针域（智能体对用户兴趣的未验证猜测，采集可顺势验证）
    let probe_domains = crate::discovery::speculator::InterestSpeculator::active_probes(char_id);
    let probe_block = if probe_domains.is_empty() {
        String::new()
    } else {
        let lines: Vec<String> = probe_domains
            .iter()
            .take(4)
            .map(|p| {
                format!(
                    "- {}（{}）",
                    p.domain,
                    p.reason.chars().take(60).collect::<String>()
                )
            })
            .collect();
        format!(
            "## 你正在验证的兴趣猜测（你猜用户可能感兴趣的方向，可围绕这些方向搜索来验证）\n{}",
            lines.join("\n")
        )
    };

    // 拼接上下文
    let context_block = if topic_block.is_empty() && memory_block.is_empty() && probe_block.is_empty()
    {
        ...
        if !probe_block.is_empty() { parts.push(probe_block); }
        parts.join("\n\n")
    };
    ...
    let system = format!(
        "你是智能体角色 {}，现在有 Busy 空闲时间，可以主动学习一些新知识或发现值得分享给用户的链接。\n\n\
         ...
         2. 其次参考「最近记忆」中的上下文补充\n\
         3. 有「兴趣猜测」时，可围绕猜测方向搜索验证（猜对了用户会很喜欢，猜错了安静退出）\n\
         4. 都没有明确锚点时，返回 `[none]`，不要硬编造话题\n\n\
         ## 分享（share）必须克制\n\
