...
impl Tool for UpdateTodoTool {
    ...
    fn description(&self) -> &str {
        "Replace the entire todo list. items: array of {title, description?, priority?, completed?, due_date?}. This fully replaces all existing todos — pass the complete desired list (existing items you want to keep must be re-included, with matching title/id). Use when reordering or restructuring the todo list."
    }

    fn description_in(&self, lang: &str) -> &str {
        match lang {
            "zh" => "整表替换待办列表。items：{title, description?, priority?, completed?, due_date?} 数组。此操作会完全替换所有现有待办——必须传入完整的目标列表（想保留的条目要重新包含）。用于重排或重构待办清单。",
            "ja" => "ToDoリスト全体を置き換える。items：{title, description?, priority?, completed?, due_date?} の配列。既存の全ToDoを完全に置き換える——完全な目標リストを渡す必要がある（残したい項目は再含めること）。並べ替えや再構成に使用。",
            _ => self.description(),
        }
    }
