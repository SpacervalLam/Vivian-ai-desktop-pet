    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Inject AppHandle into shared tool system (not dependent on character instances)
            {
                ...
                // 各创建函数内部带失败重试。
                // 角色窗口优先创建：不等 state.initialize()（约需 1 分钟），Live2D
...
/// 创建窗口，让 Live2D 模型在预加载期间立即可见。
///
/// 前端 App.tsx 的 `waitForAppReady` 机制保证 UI 在后端就绪前正常渲染
/// （Brain 相关流程等 `app:ready` 事件触发），模型 URL 由 `get_model_url`
/// 的配置回退路径提供。PetController 绑定与 presence 离线隐藏由初始化
...
pub(crate) fn create_character_windows_early(
    ...
    for entry in chars_cfg.list.iter() {
        ...
        match builder.build() {
            Ok(win) => {
                // 窗口初始可响应鼠标事件，光标追踪线程由前端 Live2DCanvas 启动
