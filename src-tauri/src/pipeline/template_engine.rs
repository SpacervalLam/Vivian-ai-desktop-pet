...
pub fn section_schema() -> PromptSectionSchema {
    let sections = vec![
        ...
        SectionDef {
            id: "first_meeting_or_rules",
            name: "First Meeting / Memory Rules",
            i18n_key: "first_meeting_or_rules",
            layer: SectionLayer::Memory,
            section_type: SectionType::Dynamic,
            optional: false,
        },
...
fn extract_section_content(id: &str, parts: &PromptParts, lang: &str) -> String {
    ...
    match id {
        ...
        "episode" => parts
            ...
            .unwrap_or("")
            .to_string(),
        "relationship_log" => parts
            .relationship_log_section
            .as_deref()
            .unwrap_or("")
            .to_string(),
        "dynamic_behavior" => parts
        ...
        "social_state" => parts
            ...
            .unwrap_or("")
            .to_string(),
        "memory" => build_memory_block(&parts.memory_text, &parts.language),
        "first_meeting_or_rules" => {
            if parts.is_first_meeting {
                first_meeting_block(&parts.language).to_string()
            } else {
                memory_usage_rules(&parts.language).to_string()
            }
        }
        "worldbook" => parts
            .worldbook_block
        ...
        "channel_guide" => {
            if parts.channel.is_empty() {
                ...
                build_channel_style_guide(&parts.channel, &parts.language)
            }
        }
        "presence_guide" => {
            if parts.presence_state.is_empty() {
                ...
                build_presence_guide(&parts.presence_state, &parts.language)
            }
        }
        "response_decision" => {
            if parts.cross_character_mode {
                ...
                if voice_guide.is_empty() {
                    cross_character_response_decision(&parts.language).to_string()
                } else {
                    format!("{}\n\n{}", voice_guide, cross_character_response_decision(&parts.language))
                }
            } else {
                user_agent_response_decision(&parts.language).to_string()
            }
        }
...
mod tests {
    ...
    fn schema_has_expected_counts() {
        let schema = section_schema();
        assert_eq!(schema.total_count, 34); // 6 static + 28 dynamic
        assert_eq!(schema.static_count, 6); // framework, output_format, character, style, examples, relationship
        assert_eq!(schema.dynamic_count, 28);
        assert_eq!(schema.optional_count, 27);
    }
