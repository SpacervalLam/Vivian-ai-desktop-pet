...
/**
 * 心情状态 - 对应 backend `get_current_mood` 返回值。
 *
 * 前 8 项来自 `psychology::mood::MoodSnapshot`（Emotion/Needs/Relationship 的实时投影，
 * 不落盘、不参与决策）；其余为命令层补齐的展示字段。两者共同构成该命令的完整契约。
 */
export interface MoodState {
  ...
  /** 唤醒度 0.0 ~ 1.0 */
  arousal: number;
  /** 主导情绪标签（7 项 EmotionLabel 之一） */
  primary_emotion: string;
  /** 次要情绪标签 */
  secondary_emotion: string;
  /** 主导情绪强度 0.0 ~ 1.0 */
  primary_intensity: number;
  /** 疲劳度 0-100（由空闲时长 + 需求未满足度推导） */
  fatigue: number;
  /** 当前压力 0-100（由负面情绪 + 安全需求推导） */
  stress: number;
  /** 关系综合分 0-100 */
  relationship_score: number;
  /** 精力 0-100 = 100 - fatigue */
  energy: number;
  /** 专注力 0-100 = arousal × 100 */
  focus: number;
  intimacy: number;
  trust: number;
