...
/**
 * AI 头像（按角色加载 public/icons/charaters/<id>.webp，URL 由后端
 * get_avatar_url 统一解析，dev/release 同源；解析失败回退 favicon）
 */
...
const ChatWindow: React.FC = () => {
  ...
  const [lastPreviews, setLastPreviews] = useState<Record<string, { content: string; timestamp: number; role: string; imagePath?: string; characterId?: string }>>({});
  ...
  const refreshLastPreviews = useCallback(async () => {
    try {
      ...
      type Preview = { content: string; timestamp: number; role: string; imagePath?: string; characterId?: string };
