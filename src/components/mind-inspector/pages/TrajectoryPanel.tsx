...
function deriveTrajectoryRecords(messages: readonly TrajectoryMessage[]): TrajectoryRecord[] {
  ...
  messages.forEach((message, index) => {
    // 跳过后端聚合落库的"工具调用意图"桩消息（role=tool_use 且无 tool_name，
    // tool_arguments 是整个 tool_calls 数组）：工具名与结果已由对应的 tool_result
    // 消息承载，若不过滤会在轨迹里出现一行无名的 '?'。
    if (message.role === 'tool_use' && !message.tool_name) return;
    // 宿主中性状态提示（上下文压缩等）不是任务轨迹，跳过；否则会落进
    // 下面的 else 分支被归成 error，在轨迹条上显示成一次「错误」。
