...
pub enum TtsEngine {
    ...
    Doubao,
    /// 小米 MiMo 语音合成(云端,语音克隆,chat/completions 接口一次性合成)
    Mimo,
...
pub struct TtsConfig {
    ...
    /// 合成超时秒数(默认 180;长文本或服务冷启动建议 300+)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gpt_sovits_timeout_secs: Option<u64>,
    /// 输出格式(对应 v2 API 的 media_type):"wav" | "raw"
    /// wav=默认无损;raw=无头 PCM,体积约减半(实验性)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gpt_sovits_format: Option<String>,
    ...
    // ── 小米 MiMo 配置（语音克隆）──
    /// MiMo API Key(api-key 请求头)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mimo_key: Option<String>,
    /// MiMo 克隆音频文件路径(本地音频,合成时 base64 内联上传)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mimo_voice_audio_path: Option<String>,
    /// MiMo 合成模型(默认 mimo-v2.5-tts-voiceclone)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mimo_model: Option<String>,
    /// MiMo 端点(默认官方 https://api.xiaomimimo.com/v1/chat/completions)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mimo_endpoint: Option<String>,
    /// MiMo 风格提示(可选,控制克隆语音的情感/语气)
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Default for TtsConfig {
    fn default() -> Self {
        Self {
            ...
            gpt_sovits_timeout_secs: None,
            gpt_sovits_format: None,
            ...
            mimo_key: None,
            mimo_voice_audio_path: None,
            mimo_model: None,
            mimo_endpoint: None,
            mimo_style_prompt: None,
            pitch: None,
...
impl TtsConfig {
    ...
    /// 这样关闭语音后遗留的服务配置不会继续常驻数 GB 模型内存。
    pub fn should_auto_start_gpt_sovits(&self) -> bool {
        self.enabled
            && self.engine == TtsEngine::GptSoVits
            && self.gpt_sovits_auto_start
            && self
    ...
    /// 是否应在应用启动时拉起 Fish Speech 本地服务。
    pub fn should_auto_start_fish_speech(&self) -> bool {
        self.enabled
            && self.engine == TtsEngine::FishSpeech
            && self.fish_speech_auto_start
            && self
...
impl TtsManager {
    ...
    fn save_to(&self) -> VivianResult<()> {
        ...
        crate::utils::fs::write_atomic(&self.persistence_path, &json)
            .map_err(|e| VivianError::Memory(format!("持久化 TTS 配置失败: {e}")))?;
        Ok(())
    }
...
mod auto_start_tests {
    ...
    fn disabled_tts_never_autostarts_gpt_sovits() {
        ...
        assert!(!config.should_auto_start_gpt_sovits());

        config.enabled = true;
        assert!(config.should_auto_start_gpt_sovits());

        config.engine = TtsEngine::EdgeTts;
        assert!(!config.should_auto_start_gpt_sovits());
    }

    #[test]
    fn disabled_tts_never_autostarts_fish_speech() {
        let mut config = TtsConfig::default();
        ...
        assert!(!config.should_auto_start_fish_speech());

        config.enabled = true;
        assert!(config.should_auto_start_fish_speech());

        config.fish_speech_install_path = Some("   ".to_string());
        assert!(!config.should_auto_start_fish_speech());
    }
}
