...
const CardBase: React.FC<CardProps> = ({
  ...
  // 可点击卡片默认可悬浮高亮；显式 hover 属性可强制开启（即使不可点击也用于视觉强调）
    ...
    <div
      ...
      style={{
        ...
        background: elevated
          ? 'var(--panel-elevated)'
          : 'var(--panel-surface)',
        border: `1px solid ${isHover ? 'var(--panel-accent)' : 'var(--panel-border-light)'}`,
        padding: SPACING.cardPadding,
        transform: isHover ? 'translateY(-2px) rotate(-0.2deg)' : 'translateY(0) rotate(0)',
        boxShadow: isHover
          ? 'var(--panel-shadow-card), 0 0 0 3px var(--panel-accent-soft)'
          : elevated
  ...
  return (
    <button
      ...
      style={{
        ...
        transition: `background ${DURATION.fast}s ${EASE.swift}, color ${DURATION.fast}s ${EASE.swift}, transform ${DURATION.fast}s ${EASE.spring}, opacity ${DURATION.fast}s ${EASE.swift}, box-shadow ${DURATION.fast}s ${EASE.swift}`,
        fontFamily: TYPO.fontFamily,
        transform: pressed && !disabled ? 'scale(0.92)' : 'scale(1)',
        boxShadow: hovered && !disabled && !active ? '0 0 0 2px var(--panel-accent-soft)' : 'none',
        ...style,
      }}
    >
      {children}
