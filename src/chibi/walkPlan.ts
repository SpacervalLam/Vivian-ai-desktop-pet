 ...
 *      位移越远 / 挪得越快，时长越短，而不是帧率去追位移。
 *   2. **腿只表达水平速度**（D）：步数由「水平位移 |dx|」推，纵向挪动交给窗口滑动本身。
 *   3. **帧间隔限幅到图集固有节奏**（A，兜底）：帧间隔夹在 [0.75, 1.35]× 原生节奏
 *      （≈58–105ms）。限幅命中时改**步数**（步幅变长）而非改时长，走动收尾仍与窗口
 *      到位同时发生，腿不滑步。
 *
 * 推导链（顺序关键）：
 *   1. slideMs = 权威时间轴（B）：距离 ÷ 速度，再限幅；
 *   2. strideFrames = round( |dx| / PX_PER_FRAME ) —— 步数由水平位移推（D）；
 *   3. frameDelay = slideMs / strideFrames，再限幅到 [A_LO, A_HI]；
 *   4. durationMs = frames × frameDelay（≈ slideMs）。
 *
 ...
 * 共用一个 core 而非各写一份，是因为「腿不滑步」这条不变式必须在两处都成立。
 */
