 ...
 * Live2D 美术资源打包加密工具
 *
 * 流程：扫描资源 → 打包 Bundle（索引+内容）→ zstd 压缩 → AES-256-GCM 加密
 * 输出：src-tauri/vivian.bundle.enc + src-tauri/asset_key.bin
 *
 * Bundle 格式（打包后、压缩前）:
 *   [4字节 magic "VBL1"]
 *   [4字节 文件数 N (LE)]
 *   [N 条索引项：每条 = 4字节路径长度 + 路径UTF8 + 8字节内容长度]
 *   [各文件内容连续拼接]
 *
 * 加密后格式:
 *   [12字节 nonce][加密数据][16字节 GCM tag]
 *
 * 用法：
...
function buildBundle(allFiles) {
  ...
  // 构建 bundle: magic + count + index + data
  const magic = Buffer.from('VBL1', 'ascii');
...
async function main() {
  ...
  const { bundle, indexMeta, entries } = buildBundle(allFiles);
  const bundleSize = bundle.length;
  console.log(`[encrypt-assets] Bundle 打包: ${entries.length} 个文件, ${(bundleSize / 1024 / 1024).toFixed(2)} MB`);

  // zstd 压缩
  console.log(`[encrypt-assets] zstd 压缩 (level ${ZSTD_LEVEL})...`);
  const { data: compressed, format: compFormat } = await zstdCompress(bundle);
  const compressedSize = compressed.length;
  const ratio = (compressedSize / bundleSize * 100).toFixed(1);
  console.log(`[encrypt-assets] 压缩后: ${(compressedSize / 1024 / 1024).toFixed(2)} MB (压缩率 ${ratio}%, 格式 ${compFormat})`);

  // AES 加密
  const encrypted = encrypt(compressed, key);
  writeFileSync(BUNDLE_FILE, encrypted);
  console.log(`[encrypt-assets] 加密完成: ${BUNDLE_FILE} (${(encrypted.length / 1024 / 1024).toFixed(2)} MB)`);

  // 输出索引 JSON（供 build.rs 读取生成 Rust 常量，含压缩格式标记）
