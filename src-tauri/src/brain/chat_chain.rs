...
pub struct BrainChatChain {
    ...
    /// 多级对话存档（压缩后的经历伪常驻于上下文，持久化跨重启）
    pub conversation_archive: Arc<RwLock<crate::memory::conversation_archive::ConversationArchive>>,
...
impl BrainChatChain {
    ...
    pub fn new(
        ...
        let conversation_archive = Arc::new(RwLock::new(
            crate::memory::conversation_archive::ConversationArchive::new(char_id),
        ));
    ...
    fn record_reasoning_trace(
        ...
        if let Some(sections_arr) = breakdown_sections {
            ...
            // Post-processing: Expression/Motion Selector (独立第二次 LLM 调用)
            {
    ...
    async fn prepare_pipeline_state(&self, user_input: &str) -> PipelineState {
        ...
        // 存档尚未形成时回退单层 TimeStampedMemory 摘要
        {
            ...
            if archive.is_empty() {
                ...
                archive.inject_into(&mut state.messages);
            }
        }
    ...
    async fn execute_pipeline_and_build_response(
        ...
        let result = self
            ...
            .scope_call_options(
                ...
                self.advisor_chain.ainvoke(state.to_json(), config),
            )
            .await?;
    ...
    async fn post_process_memory_async(
        ...
        if let Some(removed) = pending_summary {
            if !removed.is_empty() {
                ...
                // 存档时间范围：切割段首尾消息时间戳
                let archive_start = removed
                    .first()
                    .and_then(|m| m.timestamp)
                    .unwrap_or_else(chrono::Local::now);
                let archive_end = removed
                    .last()
                    .and_then(|m| m.timestamp)
                    .unwrap_or_else(chrono::Local::now);
                let retained_count = {
                ...
                if let Some(group) = pending_l2 {
                    let merged = crate::memory::conversation_archive::compress_merge_with_llm(
                        &router, &group,
                    )
                    .await;
                    ...
                    tracing::info!(
                        ...
                        group.len(),
                        group.first().map(|g| g.level).unwrap_or(1),
                        group.first().map(|g| g.level + 1).unwrap_or(2),
                    );
                }
            }
        }
