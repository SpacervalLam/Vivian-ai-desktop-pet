...
//!
//! 触发条件参考：
//! - MemoryOS: `H_segment = α·N_visit+β·L_interaction+γ·e^(-Δt/24h)` ≥ 5.0 触发三路 LLM
//! - HakusAI: `new_observations >= 3` 触发 reflection
//! - Generative Agents: importance 累计 ≥ 150 触发 reflection
//!
...
///
/// 取值 0.6 参考 memoryos-agent 的 topic merge 阈值：高于此值通常为同主题延续，
/// 低于此值视为话题切换。
...
impl ConsolidationPipeline {
    ...
    async fn stage1_summarize(&self, memory: &MemoryManager) -> VivianResult<Option<usize>> {
        ...
        // 否则新建独立 SessionSummary。
        // 参考 memoryos-agent 的 topic continuity check，避免将不同话题强行合并到同一条摘要。
    ...
    ///
    /// 四路并行设计参考 memoryos-agent：独立视角的并行分析比单次混合抽取更精准，
    ...
    ///
    /// 参考 memoryos-agent 的 `dynamic_behaviors`：从历史交互中归纳智能体的行为特征，
    /// 作为 prompt 注入的动态信号源（与 PersonaConfig 的锁定核心相对）。
