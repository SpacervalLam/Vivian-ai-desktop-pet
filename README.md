...
内置的认知调试器前端工具，通过 Memory 窗口打开（默认全屏大小），提供 3 个顶级导航页面可视化智能体内部状态与工作台：
...
├── public/                       # 静态资源（Live2D 模型 / 表情 webp / 图标 / 世界背景）
├── scripts/                      # 构建脚本（encrypt-assets 资源加密 / doctor 环境体检）
