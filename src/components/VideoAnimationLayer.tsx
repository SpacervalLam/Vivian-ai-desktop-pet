 ...
 * - 素材缺失/加载失败时优雅跳过（onerror → 淡出），不影响既有 桌宠 渲染
 * - 素材路径：`public/video-animations/<file>`（dev 由 vite 提供，prod 由 asset 协议提供）
 * - 通过 `onActiveChange` 通知父组件隐藏/恢复 桌宠 本体
 */
...
export function VideoAnimationLayer({
  ...
  useEffect(() => {
    ...
    void import('@tauri-apps/api/event').then(({ listen }) => {
      if (cancelled) return;
      // UnlistenFn 必须留存，否则组件卸载 / characterId 变更时监听器永久泄漏
      void listen<AnimPayload>('video:animation', (e) => play(e.payload)).then((fn) => {
        if (cancelled) {
          fn();
          return;
        }
      ...
      void listen<{ character_id?: string }>('video:animation:stop', (e) => stop(e.payload)).then(
        (fn) => {
          if (cancelled) {
            fn();
            return;
