...
struct ExtractedItem {
    ...
    /// Short verbatim span from the current user input supporting this fact.
    #[serde(default)]
    source_quote: String,
}
...
/// 画像补全询问的冷却时长（秒）：注入一次「自然询问」引导后，
/// 在此期间不再重复催问，避免反复查户口破坏沉浸感。24 小时。
...
struct UserFactStoreInner {
    ...
    /// 已主动引导询问过的基础字段 → 注入引导的时间戳。
    /// 用于去重：注入一次「自然询问」引导即写入，冷却期内不再催问，
    /// 避免角色每轮都追着用户问性别。用户回答后字段有值，自然不再走未知分支。
    asked_basic_fields: HashMap<UserFactType, f64>,
}
...
impl UserFactStore {
    ...
    pub async fn extract_and_upsert(
        ...
        for item in extracted.items {
            if item.confidence < 0.7 {
                tracing::debug!(
                    "[UserFactStore] 跳过低置信度事实 type={} confidence={:.2}",
                    item.item_type,
                    item.confidence
                );
                continue;
            }
            let source_quote = item.source_quote.trim().to_string();
            if source_quote.is_empty() || !user_input.contains(&source_quote) {
                tracing::warn!(
                    "[UserFactStore] 跳过缺少可核验用户原话的事实 type={}",
                    item.item_type
                );
                continue;
            }
            ...
            let fact = UserFact {
                ...
                is_pinned: false,
                // Store only the verified supporting span, not an arbitrary copy of
                // the whole user turn. This is enough for provenance and minimizes PII.
                backstory: Some(source_quote.clone()),
                person: None,
                relationship: None,
            };
    ...
    async fn upsert_fact(&self, fact: UserFact, llm: &Arc<dyn FactLlmClient>) -> VivianResult<()> {
        ...
        if fact.fact_type.is_basic() {
            ...
            // Confidence is produced by the extractor itself, not independent
            // evidence. Any conflicting value therefore goes through arbitration;
            // a self-reported 0.9 must never overwrite identity data directly.
            let resolution = self.resolve_conflict(llm, &existing, &fact).await?;
            let final_fact = match resolution.decision.as_str() {
                "choose_old" => existing,
                "choose_new" => fact,
                "merge" if !resolution.final_content.trim().is_empty() => UserFact {
                    content: resolution.final_content.trim().to_string(),
    ...
    async fn resolve_conflict(
        ...
        let payload = serde_json::json!({
            "fact_type": new_fact.fact_type.as_str(),
            "old_fact": {
                "value": old_fact.content,
                "confidence": old_fact.confidence,
                "timestamp": old_fact.timestamp,
                "source_memory_id": old_fact.source_memory_id,
                "reasoning": old_fact.reasoning,
                "pinned": old_fact.is_pinned,
            },
            "new_fact": {
                "value": new_fact.content,
                "confidence": new_fact.confidence,
                "timestamp": new_fact.timestamp,
                "source_memory_id": new_fact.source_memory_id,
    ...
    /// 生成时效标注：信息年龄超过 max_age_days 天时返回提示语，否则返回 None
    fn freshness_note(generated_at: f64, now: f64, max_age_days: i64) -> Option<String> {
        let age_secs = now - generated_at;
        if age_secs <= 0.0 {
            return None;
        }
        let age_days = (age_secs / 86400.0) as i64;
        if age_days < max_age_days {
            return None;
        }
        Some(format!("此信息已 {age_days} 天未更新，可能已过时"))
    }
    ...
    pub fn format_for_prompt(&self) -> String {
        ...
        // L0 稳定身份：5 个基础字段。
        // 性别缺失时显式标注「未知（勿猜测）」，防止模型根据姓名等线索幻觉代词；
        // 其余字段缺失则跳过（名字/年龄未知不影响代词正确性）。
        let l0_types = [
            UserFactType::Name,
            UserFactType::Age,
            UserFactType::Gender,
            ...
            for fact_type in &l0_types {
                if let Some(fact) = inner.basic_data.get(fact_type) {
                    lines.push(format!(
                        ...
                        chrono::DateTime::from_timestamp(fact.timestamp as i64, 0)
                            .map(|d| d.format("%Y-%m-%d").to_string())
                            .unwrap_or_else(|| "未知".to_string())
                    ));
                } else if *fact_type == UserFactType::Gender {
                    lines.push(format!("- {}：未知（勿猜测）", fact_type.label_zh()));
                }
            }
        }
        ...
        if l05_has_content {
            ...
            for fact_type in &l05_types {
                if let Some(fact) = inner.basic_data.get(fact_type) {
                    let source = fact.source_memory_id.as_deref().unwrap_or("user-turn");
                    lines.push(format!(
                        "- {}：{} [置信度 {:.0}%；来源记录 {}]",
                        fact_type.label_zh(), fact.content, fact.confidence * 100.0, source
                    ));
                }
            }
        }
        ...
        if l1_has_content {
            ...
            // 近期状态本就是"最近"语义：生成超过 7 天后整段标注可能过时
            if rs.generated_at > 0.0 {
                if let Some(note) = Self::freshness_note(rs.generated_at, now, 7) {
        ...
        // L2 长期事实：自由事实（超过 30 天的旧事实逐条标注时效，防止以现在时态误用）
        if !inner.custom_facts.is_empty() {
            lines.push(String::new());
            lines.push("【关于用户的其他事实】".to_string());
            for fact in inner.custom_facts.iter().take(10) {
                let stale = Self::freshness_note(fact.timestamp, now, 30)
                    .map(|note| format!("（⚠ {}）", note))
                    .unwrap_or_default();
                let source = fact.source_memory_id.as_deref().unwrap_or("user-turn");
                lines.push(format!(
                    "- {}{stale} [置信度 {:.0}%；来源记录 {}]",
                    fact.content, fact.confidence * 100.0, source
                ));
            }
        }

        // Missing demographic data is a guardrail, not a reason to mutate state or
        // pressure the user for disclosure. Actual questions should be recorded only
        // when the assistant really asks them.
        let gender_missing = !inner.basic_data.contains_key(&UserFactType::Gender);
        if gender_missing {
            lines.push(String::new());
            lines.push(
                "（系统提示：用户性别未知。用「你」指代，不根据姓名、语气等线索猜测，也不要主动索取该信息。）"
                    .to_string(),
            );
        }
...
impl UserFactStoreInner {
    fn load(&mut self) -> VivianResult<()> {
        ...
        self.asked_basic_fields = data.asked_basic_fields;
        Ok(())
    }
    ...
    fn save(&self) -> VivianResult<()> {
        ...
        let data = serde_json::json!({
            ...
            "asked_basic_fields": &self.asked_basic_fields,
        });

        let json = serde_json::to_string_pretty(&data).unwrap_or_default();
        // 原子写：自带 fsync + UUID 临时名，比手写 tmp + rename 多一层断电保护与并发安全
        crate::utils::fs::write_atomic(&self.store_path, &json)
...
fn build_extract_prompt(
    ...
    let body = match lang_norm {
        "en" => format!(
            ...
            - 0.7-0.8: explicitly stated but context or durability is somewhat ambiguous\n\
            - Below 0.7: do not extract; never store inferred or implicit identity facts\n\n\
            Every item must include source_quote: the shortest verbatim span copied from the current user input that directly supports the fact.\n\
            Output only JSON, no markdown code blocks:\n\
            {{\"detected\":true,\"items\":[{{\"type\":\"name\",\"content\":\"Xiaoming\",\"confidence\":0.95,\"reasoning\":\"user self-introduction\",\"source_quote\":\"My name is Xiaoming\"}}]}}",
        ...
        "ja" => format!(
            ...
            - 0.7-0.8：明示されたが、文脈や長期性に多少の曖昧さがある\n\
            - 0.7 未満：抽出しない。推測・暗黙の個人属性は保存しない\n\n\
            各項目の source_quote に、その事実を直接裏付ける今回のユーザー入力の最短原文を必ずコピーする。\n\
            JSON のみを出力し、markdown コードブロックは使用しない：\n\
            {{\"detected\":true,\"items\":[{{\"type\":\"name\",\"content\":\"小明\",\"confidence\":0.95,\"reasoning\":\"ユーザーの自己紹介\",\"source_quote\":\"小明です\"}}]}}",
        ...
        _ => format!(
            ...
            ## 置信度\n\
            - 0.9+：用户明确自我介绍（\"我叫X\"）\n\
            - 0.7-0.8：用户明确说出，但语境或长期有效性略有歧义\n\
            - 低于 0.7：不要抽取；禁止保存推断或隐含的身份事实\n\n\
            每项必须包含 source_quote：逐字复制本轮用户输入中直接支持该事实的最短原话。\n\
            只输出 JSON，不要 markdown 代码块：\n\
            {{\"detected\":true,\"items\":[{{\"type\":\"name\",\"content\":\"小明\",\"confidence\":0.95,\"reasoning\":\"用户自我介绍\",\"source_quote\":\"我叫小明\"}}]}}",
