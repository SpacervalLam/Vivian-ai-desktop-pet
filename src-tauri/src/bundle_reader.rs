...
//! Bundle 文件由资源加密步骤生成（VBL2 per-file 格式），索引由 build.rs 嵌入二进制。
...
/// 解压后热资源缓存容量上限
///
/// 16MB 足以容纳全部热资源（模型 json / 表情 / 图标）。纹理已降至 2048²
/// （单张解压后 16MB），本来就不该进这个缓存——走按需读取即可，
/// 否则一张纹理就会把整个缓存挤空。
...
pub fn init(bundle_path: &PathBuf) -> Result<(), String> {
    ...
    if magic == b"VBL1" {
        return Err(
            "检测到旧版 VBL1 bundle（整体解压常驻内存格式），请重新生成 VBL2 按需解压格式的 bundle（资源加密步骤产物）".to_string(),
        );
    }
    if magic != b"VBL2" {
    ...
    if BUNDLE_ENTRIES.len() != count {
        return Err(format!(
            "bundle 与编译期索引不匹配: 磁盘 {} 条, 编译期 {} 条 (请重新生成 bundle 索引并重新编译)",
            count,
            BUNDLE_ENTRIES.len()
        ));
    }
    ...
    for entry in BUNDLE_ENTRIES.iter() {
        match disk_index.get(entry.path) {
            ...
            _ => {
                return Err(format!(
                    "bundle 索引条目不匹配: {} (请重新生成 bundle 索引并重新编译)",
                    entry.path
                ));
            }
        }
...
/// 根据路径获取资源明文字节
///
/// 路径格式: "Vivian/vivian-atlas.png"（相对资源根目录的虚拟路径）。
