...
/// 生成紧凑的参数预览（key=value 连接，单值截断 40 字符，总长截断）
fn compact_args_preview(args: &serde_json::Value, max_len: usize) -> String {
    ...
    let mut parts: Vec<String> = obj
        ...
        .map(|(k, v)| {
            ...
            let val = if collapsed.chars().count() > 40 {
    ...
    if joined.chars().count() > max_len {
        joined = format!("{}…", joined.chars().take(max_len).collect::<String>());
    }
    joined
}
...
pub fn confirmation_info(tool_name: &str, args: &serde_json::Value) -> (ConfirmationRisk, String) {
    ...
    match tool_name {
        ...
        "create_tool" => {
            ...
            // （名称/描述/参数 schema/脚本全文/权限等级/动态注入等级）
            ...
            (
                ...
                if desc.is_empty() {
                    format!("请求创建新工具「{name}」，请在预览卡片中审核后决定")
                } else {
                    format!("请求创建新工具「{name}」：{desc}")
                },
            )
        }
        "create_plugin" => {
            // 插件创建确认：预览卡片展示各贡献点概要（MCP 命令行与工具脚本
            // 全文由前端卡片渲染——那是 shell 级敏感点，必须逐条可见）
            let name = args.get("name").and_then(|v| v.as_str()).unwrap_or("?");
            let skills = args.get("skills").and_then(|v| v.as_array()).map(|a| a.len()).unwrap_or(0);
            ...
            let parts: Vec<String> = [
                (skills > 0).then(|| format!("{skills} 条技能")),
                (tools > 0).then(|| format!("{tools} 个工具")),
                (mcp > 0).then(|| format!("{mcp} 个 MCP server")),
                (providers > 0).then(|| format!("{providers} 条供应商预设")),
            ]
            .into_iter()
            .flatten()
            .collect();
            (
                ConfirmationRisk::High,
                format!(
                    "请求创建插件「{name}」（{}），预览卡片将展示 MCP 命令行与工具脚本全文，请审核后决定",
                    if parts.is_empty() { "无贡献点".into() } else { parts.join("、") }
                ),
            )
        }
        "browser_navigate" => {
            let url = args.get("url").and_then(|v| v.as_str()).unwrap_or("?");
            (ConfirmationRisk::Medium, format!("将打开网站：{}", url))
        }
        "browser_task_tab" => {
            let url = args.get("url").and_then(|v| v.as_str()).unwrap_or("?");
            (
                ConfirmationRisk::Medium,
                format!("将在后台标签页打开：{}", url),
            )
        }
        "browser_click" => {
            let index = args.get("index").map(|v| v.to_string()).unwrap_or_else(|| "?".into());
            (
                ConfirmationRisk::Medium,
                format!("将点击网页元素（编号 {}）", index),
            )
        }
        "browser_type" => {
            ...
            (
                ConfirmationRisk::Medium,
                format!("将在网页输入文本：{}", text),
            )
        }
        "browser_press" => {
            let key = args.get("key").and_then(|v| v.as_str()).unwrap_or("?");
            (ConfirmationRisk::Medium, format!("将发送按键：{}", key))
        }
        "browser_scroll" => {
            let dir = args
                .get("direction")
                .and_then(|v| v.as_str())
                .unwrap_or("?");
            (ConfirmationRisk::Low, format!("将滚动网页（方向：{}）", dir))
        }
        "browser_eval_js" => (
            ConfirmationRisk::High,
            "将在网页中执行 JavaScript 代码".to_string(),
        ),
        "take_screenshot" | "screenshot_analyze" => {
            (ConfirmationRisk::Medium, "将截取当前屏幕".to_string())
        }
        "manage_scheduled" | "cancel_scheduled" => {
            ...
            (
                ConfirmationRisk::Medium,
                format!("将{}定时任务（ID {}）", action, id),
            )
        }
        "manage_todo" | "delete_todo" => {
            let id = args.get("id").and_then(|v| v.as_str()).unwrap_or("?");
            let action = match args.get("action").and_then(|v| v.as_str()) {
                Some("delete") | None => "删除",
                Some("update") => "更新",
            ...
            (
                ConfirmationRisk::Medium,
                format!("将{}待办事项（ID {}）", action, id),
            )
        }
        _ => {
            let preview = compact_args_preview(args, 120);
            if preview.is_empty() {
                (ConfirmationRisk::Low, format!("工具 {} 请求执行", tool_name))
            } else {
                (
                    ConfirmationRisk::Low,
                    format!("工具 {} 请求执行（参数：{}）", tool_name, preview),
                )
            }
        }
    }
