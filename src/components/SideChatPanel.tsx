...
export default function SideChatPanel() {
  ...
  useEffect(() => {
    void invoke('set_side_chat_input_open', { open: inputVisible }).catch(() => {});
  }, [inputVisible]);
  ...
  const handleEscape = useCallback(() => {
    if (lockedRef.current) {
      void invoke('set_side_chat_locked', { locked: false }).catch(() => {});
    }
  }, []);

  // 双击 header 切换锁定：锁定后窗口常驻，解锁后光标离开自动隐藏。
  ...
  const toggleLock = useCallback(() => {
    ...
    void invoke('set_side_chat_locked', { locked: !lockedRef.current }).catch(() => {});
  }, [inputVisible]);

  return (
    <div
      onDoubleClick={toggleLock}
      title={locked ? '双击解锁（光标离开自动隐藏）' : '双击锁定（常驻）'}
      style={{
        width: '100%',
        height: '100vh',
        display: 'flex',
