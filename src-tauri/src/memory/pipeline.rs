...
impl ConsolidationPipeline {
    ...
    ///
    /// 从会话摘要中归纳当前角色已表现出的稳定行为模式，作为可演化的动态人设层。
    /// 与路径 3（行为模式抽取）的区别：路径 3 抽取的是**用户**的行为模式，
    /// 本路径抽取的是**当前角色自己**已表现出的语言风格、行为举止、互动方式、习得能力。
    ///
    ...
    async fn stage2_extract_dynamic_persona(
        ...
        let prompt = format!(
            "请从以下会话摘要中归纳 **当前角色自己** 已表现出的稳定行为模式。\n\n\
             ## 不可修改的人设核心\n\
             {}\n\
             ## 任务\n\
             抽取四类语义级行为：\n\
             ...
             \
             ## 抽取原则\n\
             - 只抽取当前角色**已经表现出**的行为，不把用户的偏好、情绪或行为误记成角色习惯\n\
             - 仅记录多次出现且彼此独立、或单次明确纠正后可复用的模式；偶发行为不算成长\n\
             - 用户表达的不满/边界只能转成具体的尊重方式，不能扩写成讨好、依附或新的关系事实\n\
             - 不抽取核心身份、世界观、价值观、能力边界；工具存在也不等于角色已经学会或总会使用\n\
             - 描述应短、具体、可执行，并说明适用情境；避免空泛特质或命令式固定表演\n\
             - 每类最多抽取 2 条；证据不足时返回空数组\n\
             \
             ## 输出格式\n\
