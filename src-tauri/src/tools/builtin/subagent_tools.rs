...
impl Tool for SubagentReportTool {
    ...
    fn usage_corpus(&self, lang: &str) -> &'static str {
        match lang {
            "zh" => "子代理报告了什么\n看看它的结果",
            "en" => "what did the subagent report\nshow its result",
            "ja" => "サブエージェントの結果",
            _ => "",
        }
    }
...
impl Tool for SubagentControlTool {
    ...
    /// 权限：`subagent_control` 只操作智能体自己的任务登记表（查询进度 / 取消 /
    /// 延续 / 取报告），不触碰文件系统、Shell、网络或输入设备，因此不弹用户确认。
    /// 真正需要把关的是「新起一个子代理」——那道闸在 `spawn_subagent` 上。
    async fn check_permissions(&self, _input: &Value, _ctx: &ToolUseContext) -> PermissionResult {
        PermissionResult::allow()
    }
    ...
    /// 权限风险等级：仅操作智能体自身的任务登记表（内存态），
    /// 不写盘、不起进程、不联网、不模拟输入 → Safe。
    /// 与 `spawn_subagent`（Shell，需确认）区分：那道闸拦的是「新起子代理」。
    fn risk(&self) -> ToolRiskTier {
        ToolRiskTier::Safe
    }
