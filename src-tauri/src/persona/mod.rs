...
pub struct PersonaEngine {
    ...
    /// 人设配置修订号（任何 config 变更时递增，驱动 Character 块渲染缓存失效）
    config_revision: std::sync::atomic::AtomicU64,
    /// Character 块渲染缓存：(tier, lang, config修订, evolution更新时间, 渲染文本)
    character_block_cache: RwLock<Option<(prompt_render::CharacterBlockTier, String, u64, f64, String)>>,
}
...
impl PersonaEngine {
    ...
    pub fn new(char_id: &str) -> VivianResult<Self> {
        ...
        Ok(Self {
            ...
            character_block_cache: RwLock::new(None),
        })
    }
    ...
    fn save_to(&self) -> VivianResult<()> {
        ...
        crate::utils::fs::write_atomic(&self.persistence_path, &json)
            .map_err(|e| VivianError::Memory(format!("持久化人格配置失败: {e}")))?;
        Ok(())
    }
    ...
    pub fn set_section_definition(&self, section_key: &str, body: &str) -> VivianResult<()> {
        ...
        };
        {
            ...
            let field = match section {
                ...
                CharacterSection::PersonaConfig => unreachable!(),
            };
            *field = text;
        }
        self.bump_config_revision();
    ...
    pub fn get_character_block(&self) -> String {
        self.get_character_block_tiered(prompt_render::CharacterBlockTier::Full)
    }

    /// 按熟悉度档位渲染 Character 块（含渲染缓存）
    ///
    /// 熟客档（Compact）裁掉 Background / Interests / Appearance 等自我介绍型段落。
    /// 渲染结果按 (tier, lang, config版本, evolution版本) 缓存：人设/语言/覆盖层未变时
    /// 直接复用上次拼接结果，避免每轮对话重复拼接大字符串（输出字节稳定，缓存前缀安全）。
    pub fn get_character_block_tiered(
        &self,
        tier: prompt_render::CharacterBlockTier,
    ) -> String {
        let lang = self.language.read().clone();
        let evolution_version = self.evolution.last_update();
        let cfg_ver = self
            .config_revision
            .load(std::sync::atomic::Ordering::Relaxed);
        {
            let cache = self.character_block_cache.read();
            if let Some((c_tier, c_lang, c_cfg_ver, c_evo_ver, ref text)) = cache.as_ref() {
                if *c_tier == tier
                    && *c_lang == lang
                    && *c_cfg_ver == cfg_ver
                    && *c_evo_ver == evolution_version
                {
                    return text.clone();
                }
            }
        ...
        *self.character_block_cache.write() = Some((tier, lang, cfg_ver, evolution_version, block.clone()));
        block
    }
