...
impl OllamaServiceManager {
    ...
    pub async fn start(&self, ollama_path: &str) -> VivianResult<OllamaServiceState> {
        ...
        // 优先检测端口已监听的 Ollama（外部启动的，或上一生命周期留下的常驻
        // 实例）：直接复用，不做孤儿清理——常驻实例是刻意存活的，不是孤儿
        if Self::check_port().await {
            state.status = OllamaServiceStatus::Running;
            state.error = None;
            state.pid = None;
            tracing::info!("[OllamaService] 检测到 Ollama 已在运行，直接复用");
            let ret = state.clone();
        ...
        // 端口未监听：清理上次残留的进程（可能僵死占资源但端口未就绪）
        pid_file::cleanup_orphan("ollama", "ollama.exe");
        ...
        cmd.arg("serve");
        // 常驻服务：应用退出/Child 意外 drop 均不杀进程，仅 stop() 手动停止
        ...
        let pid = child.id();
        // 不绑定 Job Object：Ollama 需在应用退出（含崩溃/强杀）后继续存活，
        // 下次启动复用，避免嵌入服务冷启动等待
        if let Some(pid) = pid {
    ...
    pub async fn ensure_model_installed(model: &str, ollama_path: &str) -> bool {
        ...
        // 先等待 HTTP API 真正就绪再检查模型，否则连接失败会被误判为
        // "未安装"，触发对已安装模型的无意义 pull；同时保证返回 true 时
        // 嵌入 API 已可用，调用方随即开始的嵌入请求不会因服务未就绪失败。
        if !Self::wait_for_api_ready(std::time::Duration::from_secs(30)).await {
            tracing::warn!(
                "[OllamaService] 等待服务 API 就绪超时，跳过模型 {} 安装检查",
                model
            );
            return false;
        }
    ...
    /// 轮询等待 HTTP API 真正可用（GET /v1/models 返回可解析 JSON），超时返回 false
    ///
    /// TCP 端口可达不代表 API 就绪（serve 进程启动初期端口监听后 API 仍需
    /// 初始化时间），嵌入请求若在 API 就绪前发出会连接失败。启动流程必须
    /// 以此函数确认就绪后再开始嵌入。
    async fn wait_for_api_ready(timeout: std::time::Duration) -> bool {
