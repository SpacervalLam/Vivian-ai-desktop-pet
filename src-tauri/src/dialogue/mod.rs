...
impl DialogueManager {
    ...
    pub fn flush_buffer(&self) -> VivianResult<()> {
        ...
        // 2. 读取现有消息
        let mut existing = self.read_all_messages();

        // 3. 重复检测并追加
        let mut added = 0;
        for msg in &messages_to_write {
            if !Self::is_duplicate(msg, &existing) {
        ...
        // 4. 原子写入
        match self.write_all_messages(&existing) {
            Ok(_) => {
                tracing::debug!(
                    "已写入 {} 条消息到磁盘（共 {} 条）",
                    added,
    ...
    fn backup_corrupted_file(path: &Path) {
        let timestamp = std::time::SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let backup_path = path.with_extension(format!("json.corrupt_{}", timestamp));
        if path.exists() {
            if let Err(e) = fs::rename(path, &backup_path) {
                tracing::error!("备份损坏文件失败: {}", e);
                return;
            }
            tracing::warn!("损坏文件已备份到: {:?}", backup_path);
        }
        // 创建新的空文件（新格式）
        let file_data = HistoryFile::default();
        match serde_json::to_string_pretty(&file_data) {
            Ok(json) => {
                if let Err(e) = fs::write(path, json) {
                    tracing::error!("创建新历史文件失败: {}", e);
                }
            }
            Err(e) => tracing::error!("序列化空历史文件失败: {}", e),
        }
    }

    /// 原子写入所有消息（tmp + rename）
    fn write_all_messages(&self, messages: &[HistoryEntry]) -> VivianResult<()> {
        let path = self.history_file();
    ...
    pub fn patch_last_assistant_timestamp(&self, timestamp_ms: i64) {
        ...
        match std::fs::read_to_string(&path) {
            ...
            Err(e) => {
                tracing::warn!("[DialogueManager] 读取历史文件 patch assistant timestamp 失败: {}", e);
            }
        }
    }
    ...
    pub fn patch_last_user_timestamp(&self, queued_at: chrono::DateTime<chrono::Local>) {
        ...
        match std::fs::read_to_string(&path) {
            ...
            Err(e) => {
                tracing::warn!("[DialogueManager] 读取历史文件 patch user timestamp 失败: {}", e);
            }
        }
    }
