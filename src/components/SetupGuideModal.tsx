...
const SetupGuideModal: React.FC<SetupGuideModalProps> = ({
  ...
  const statusMeta = (ok: boolean | undefined) => {
    const isOk = ok === true;
    return {
      label: isOk
        ? tr('config.setup_guide.configured')
        : tr('config.setup_guide.not_configured'),
      color: isOk ? '#2E7D32' : '#C62828',
      bg: isOk ? 'rgba(46, 125, 50, 0.12)' : 'rgba(198, 40, 40, 0.10)',
    };
  };

  const pathCard = (path: string, tag: string, tagKey: string, desc: string, configured?: boolean) => (
    <div
      style={{
        padding: '10px 12px',
        borderRadius: 8,
        background: C.cardBg,
        border: `1.5px solid ${configured === true ? 'rgba(46,125,50,0.35)' : C.border}`,
        marginBottom: 8,
      }}
    >
      <div
          ...
          {configured !== undefined && (
            <span
              style={{
                fontSize: 11,
                fontWeight: 600,
                color: statusMeta(configured).color,
                background: statusMeta(configured).bg,
                borderRadius: 4,
                padding: '1px 6px',
                lineHeight: 1.5,
                whiteSpace: 'nowrap',
