...
export function useMicroPresence(
  ...
  useEffect(() => {
    ...
    const scheduleGaze = (t: number, stability: number) => {
      ...
      // 远端视线目标时，头部以一定概率顺势协同转向（模拟真人随机张望时眼先动、头跟上）
      if (!stretch.active && Math.abs(p.x) >= 0.45 && Math.random() < 0.6) {
        headFromX = headX;
        headToX = p.x * (16 + Math.random() * 6);  // 约 9-19°，随视线方向
        headMoveStart = t;
        headMoveDuration = 1.2 + Math.random() * 0.8;
        headHoldUntil = t + headMoveDuration + 5 + Math.random() * 7;
      }
    };
