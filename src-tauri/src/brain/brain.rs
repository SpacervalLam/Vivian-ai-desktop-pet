...
impl Brain {
    ...
    async fn build(
        ...
        // 各角色通过 set_foreground_listener 订阅前台窗口事件，独立写入各自的 user_behaviors.json
        ...
        // 避免重复调用分类器。
        {
            ...
            world_provider.set_foreground_listener(char_id, std::sync::Arc::new(move |fw| {
        ...
        // 注入 Mind 启用 Stage 4（Belief/Goal 生成）；按角色隔离健康持久化文件
        ...
        // 启动预加载：情绪/语义语料嵌入必须完成后再开放 API（见 lib.rs 的 app:ready 时序）
        if let Err(e) = brain.preload_perception() {
    ...
    /// 预加载情绪感知与快速语义语料嵌入。
    ///
    /// 这些模块默认懒加载，若不预加载，首个 API 请求会阻塞在语料嵌入上，
    /// 甚至因并发初始化窗口返回“语料嵌入正在初始化中”错误。
    /// 预加载失败会返回 Err，由上层决定是否跳过该角色或阻止启动。
    pub fn preload_perception(&self) -> Result<(), String> {
        // 以当前进度为基点做区间映射：多角色依次预加载时百分比单调推进不回退
        let base = crate::startup::last_percent().max(75);
        crate::startup::emit_progress(base, 100, "正在嵌入情绪感知语料…");
        if let Some(ref classifier) = self.embedding_classifier {
            classifier.set_progress_callback(Arc::new(move |completed, total| {
                let pct = base + ((completed * 5) / total.max(1)).min(5);
                crate::startup::emit_progress(
                    pct,
                    100,
                    &format!("正在嵌入情绪感知语料… {}/{}", completed, total),
                );
            }));
            if let Err(e) = classifier.preload() {
                tracing::warn!("[Brain:{}] 情绪语料嵌入预加载失败: {}", self.char_id, e);
                return Err(format!("情绪语料嵌入预加载失败: {}", e));
            }
        }
        let semantic_base = crate::startup::last_percent().max(80);
        crate::startup::emit_progress(semantic_base, 100, "正在嵌入语义理解语料…");
        if let Some(ref fast_semantic) = self.fast_semantic {
            if let Err(e) = fast_semantic.preload() {
                tracing::warn!("[Brain:{}] 快速语义语料嵌入预加载失败: {}", self.char_id, e);
                return Err(format!("快速语义语料嵌入预加载失败: {}", e));
            }
        }
        Ok(())
    }
    ...
    pub async fn generate_startup_greeting(&self) -> Option<String> {
        ...
        // 距上次交流过去了多久：让开场能区分"刚出去五分钟"和"一整天没动静"，
        // 而不是一律套用"回来了"这个万能起手式。
        let away_brief = {
            let last_ts = self
                .dialogue
            ...
            match last_ts {
                Some(ts) => {
                    let elapsed = chrono::Local::now().signed_duration_since(ts);
                    ...
                    format!("- Time since you last talked: {}", span)
                }
                None => "- Time since you last talked: 很久（没有可查的记录）".to_string(),
            }
        };
    ...
    async fn build_startup_greeting_memory_text(&self) -> String {
        ...
        // 历史启动问候：注入最近 6 条，让 LLM 避免重复相同开场。
        // 过去只取 3 条，而模型最常复用的恰恰是"回来了 + 吃了吗"这类组合，
        // 3 条的窗口太窄，隔几轮就滑出视野，于是同一句开场在一天内反复出现。
        let recent_greetings = self.memory.recent_by_tags(&["startup_greeting"], 6);
        if !recent_greetings.is_empty() {
            ...
            sections.push(format!(
                "## 最近用过的开场（DO NOT repeat these — vary your wording）\n\
                 {}\n\
                 注意：这些开场里反复出现的起手式（回归词、问吃饭等）本身就是模板信号，\
                 这一轮必须换个开口方式——从别的事情切入。",
                parts.join("\n")
            ));
        }
    ...
    pub fn build_prompt_template_preview(&self) -> crate::mind::reasoning_trace::PromptBreakdown {
        ...
        sections.push(("Framework (Safety)", crate::pipeline::prompt_modules::safety_rules().to_string()));
        sections.push(("Framework (Address Rules)", crate::pipeline::prompt_modules::address_rules().to_string()));
        sections.push(("Framework (Conversation Rhythm)", crate::pipeline::prompt_modules::conversation_rhythm().to_string()));
        sections.push(("Framework (Session Rules)", crate::pipeline::prompt_modules::session_rules().to_string()));
        sections.push(("Framework (Output Format)", crate::pipeline::prompt_modules::output_format().to_string()));
        sections.push(("Framework (Speaker Prefix)", crate::pipeline::prompt_modules::speaker_prefix().to_string()));
...
that selects expression and motion based on the conversation.\n\n\
--- System Prompt ---\n{}\n\n\
...
Choose the appropriate expression and motion. Leave empty if nothing fits.",
            emote_prompt
        );
        sections.push(("Post-processor: Expression & Motion", emote_section_content));
            ...
            let (section_id, layer, optional) = match schema_map.get(name_str) {
                ...
                None => {
                    if name_str.contains("Post-processor") {
                        ("post_expression_motion".to_string(), "postprocess".to_string(), true)
